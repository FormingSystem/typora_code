...
提交右键可打开全部改动或按远端平台识别的托管网页（GitHub、Gitee、GitLab、Bitbucket），切换关联分支／远端引用、分离 HEAD、创建分支或标签、删除关联分支、挑选提交、比较上游／基线／指定引用，以及复制完整提交消息或哈希。比较使用固定 Git 对象；没有上游或基线时禁用对应比较。其他既有 Git 操作保留在子菜单中。
...
## 2026-09-14 Git刷新触发与无变化更新

用户指出Git区域反复刷新，并明确要求首次打开刷新、持续停留时每15至30分钟检查。本次采用30分钟可见检查间隔；这是产品约定，不是VS Code默认值。首次打开立即读取，同一仓库已有在途读取时复用；再次切回、窗口聚焦、同仓库文件标签切换不无条件重读。隐藏期间停止定时检查，恢复可见后只补到期或已记录的变更。手动刷新和提交、获取、拉取、推送等命令完成仍立即读取，失败同样更新可观察结果，普通操作不增加确认。

固定VS Code 1.137.0的[repository.ts:3178](https://github.com/microsoft/vscode/blob/645f29cc3176500b4b5762ba887cf2a7f0ffdf2c/extensions/git/src/repository.ts#L3178)使用文件事件、1000ms防抖和操作空闲／窗口聚焦条件，执行后等待5000ms；该等待不是定时轮询。[extHostSCM.ts:661、886](https://github.com/microsoft/vscode/blob/645f29cc3176500b4b5762ba887cf2a7f0ffdf2c/src/vs/workbench/api/common/extHostSCM.ts#L661)只发布非空资源和引用差量。Git Graph v1.30.0（`881a9e613045bacbbadf8940f6b6c5b8bd699335`）的[repoFileWatcher.ts:68](https://github.com/mhutchie/vscode-git-graph/blob/v1.30.0/src/repoFileWatcher.ts#L68)使用750ms事件防抖，`gitGraphView.ts`随可见性启停监听，`web/main.ts`软刷新比较提交和引用后决定是否重绘。自动fetch是另一种联网行为，不能用它的间隔代替本地状态刷新。

工作台绑定拥有可见仓库的自动检查调度；控制器拥有读取事务、最近刷新时刻和仓库快照。已落盘的保存通知按1000ms合并，且距上次读取完成至少5000ms，与在途读取或写操作串行；窗口焦点仅唤醒待处理任务，不生成额外刷新。底栏订阅同一快照，取消自己的8秒轮询和对面板`data-state`的反向刷新；当前仓库行复用该快照。原生Markdown的didSave、源码及可编辑diff复用文件保存完成事件，不在写盘前按固定延时猜测完成。连续批量保存最多补读一次，不取消已在途读取。自动读取延续当前筛选、加载数量和详情，不增加布局占位。

结果相同时保留提交行、文件行、焦点、选择、展开和滚动；文件状态仍为M但内容变化时，工作区／暂存区比较仍重新取内容，不能以状态字母不变推断正文不变。切换仓库即使HEAD和文件组相同，也重新应用该仓库的树形、排序与折叠布局；子目录正规化为仓库根后再创建其他仓库摘要的读取批次。图标命令的移动进度条继续绑定实际Promise；无操作的空闲窗口不闪动。切库和销毁清理订阅、定时器及迟到结果。本轮不新增后台自动联网或移植上游扩展宿主，外部磁盘变更由30分钟检查或用户手动刷新读取，宿主保存和本产品Git操作及时更新。

验收用专属临时仓库和可控时钟，覆盖首次／重复打开、30分钟边界、隐藏恢复、聚焦、批量保存、手动／命令后刷新、错误与销毁；真实Git核对无变化DOM身份和焦点，以及M→M比较内容更新。安装后的资产校验与运行窗口加载状态分别记录。
...
提交菜单与浮层使用共同的[托管网页设计](git_commit_web.md)，按远端识别GitHub、Gitee、GitLab和Bitbucket；不再维护GitHub专用拼接函数。多目标一次选择、远端复核和切库失效由共同动作管理。

## R027.1 提交图传入与传出更改

2026-09-24用户要求补齐Outgoing Changes分支行。现有底栏领先/落后计数没有对应图节点；新增传出更改和传入更改，表示当前分支与实际配置上游各自在共同祖先后的提交，展开比较共同祖先到对应端点的汇总文件，不混入未提交改动。分支名来自真实引用，不假定master/main或origin，计数以本地最后获取的远端跟踪引用为准；显示本身不自动联网。用户获取/推送/拉取后沿既有刷新更新。

固定VS Code研究基线68070681e87284e2f22728f15fe3f3651fbf932b：scmHistory.ts的addIncomingOutgoingChangesHistoryItems在HEAD前放传出节点、共同祖先前放传入节点；scmHistoryViewPane.ts在子节点读取时比较共同祖先与本地/远端引用；scm.contribution.ts两种显示默认均true。采用相同默认、22px侧栏行、11px轨道及drawDashedCircle的虚线圆，颜色/悬停/选中复用共享状态。当前不增加新的设置页；顶栏拉取推送沿已有命令。

仓库读取服务在同一快照读取共同祖先；共享图投影拥有虚拟区间身份和拓扑，侧栏/编辑区Graph共用。区间ID不得作为Git版本、提交右键操作或提交悬停查询；差异仍携带真实root/from/to/file身份。已选文件与区间/提交选择互斥，展开不是第二条选中。文件列表按需加载并缓存不可变端点，换仓、收起或刷新使迟到结果失效。

只有实际端点存在、对应引用在当前筛选内且方向计数非零时显示；传入节点还要求共同祖先已载入，翻页后出现。无上游、上游已删除、分离HEAD、未出生分支或无共同祖先不伪造节点，不阻断已有历史。Git读取失败由原仓库错误机制反馈，不把失败当作同步。验证使用临时双克隆和bare远端：一致、仅领先、仅落后、分叉、异名上游、获取/推送后刷新、无上游、筛选/分页及过期返回；UI覆盖展开、汇总diff、选择、键盘和明暗节点形状，同候选安装与卸载。

## R027.2 分支与共同历史颜色

2026-09-25：此前颜色只按拓扑轨道递增，其他分支沿第一父链经过当前HEAD时没有引用身份边界，造成独有提交与当前历史同色。目标是按引用角色和拓扑共同着色，不把分支标签颜色或选中底色代替线与节点颜色。

固定VS Code `68070681e87284e2f22728f15fe3f3651fbf932b` 的 `scmHistoryViewPane.ts::_getGraphColorMap`、`scmHistory.ts::getLabelColorIdentifier/toISCMHistoryItemViewModelArray` 定义当前引用chartsBlue、上游chartsPurple、基准#EA5C00；其他轨道依次#FFB000/#DC267F/#994F00/#40B0A6/#B66DFF。采用其引用边界与调色板：其他分支独有段从黄色起，遇当前HEAD改用蓝色；已知上游与基准分别用紫/橙色。同一SHA优先当前HEAD，其次上游、基准，避免同节点多色争抢；分离HEAD仍有当前身份。汇合前各条入线保留原色，到节点才汇入所选历史颜色。

`build_history_model`从同一仓库快照生成提交颜色角色，`build_git_graph`负责节点/上下半边的色值编号；侧栏、完整Graph及展开文件续线读取同一结果。无额外Git访问、异步状态或独立UI颜色缓存；筛选/分页/刷新重建时读取新快照。区间与工作区虚拟节点继承其业务角色，不占普通分支颜色序列。已有用户自定义颜色继续按槽位映射；选择/焦点/悬停与HEAD圆形几何维持原所有者。

范围仅提交图着色，不改变分支、提交、选择或正文主题。没有已知引用时仍绘制普通轨道，不推测远端或触发fetch；原读取失败/取消保留现有反馈。回归使用真实临时Git分支与bare远端，覆盖HEAD上方其他分支的线性延伸、双向分叉、合并、同点引用、分页、分离HEAD、20/100/1000节点；Chromium/原始宿主核对两种图视图和展开续线，记录明暗及自定义颜色边界。
