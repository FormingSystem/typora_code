...
Changes标题、Graph标题和中央Graph工具栏复用同一覆盖进度组件；绝对定位在标题下沿，不增加行高，不接收鼠标。底栏和仓库行从同一活动读取提示与忙碌，网络过程使用官方sync图标旋转；其余动作保留自身图标并提供操作提示。设置`show_progress`对应git.showProgress，默认true，只控制进度条，不能隐藏业务锁和失败信息。未知工作总量不显示百分比；减少动画设置使用静态进度提示。
