...
| 区域 | 保留行为 | 已撤去的本轮扩展 |
| --- | --- | --- |
| 标题栏 | Typora 标准原生窗口、文件/编辑/段落/格式/视图/主题/帮助七个系统菜单；快速打开位于标签条右侧 | Command Center、自绘菜单重组、导航及布局按钮 |
...
| 对象 | 当前冻结约定 | 来源与保留修复 |
| --- | --- | --- |
| 系统标题栏 / 窗控 | 由 Typora 标准原生窗口管理 | 无自绘 DOM、图标比例或固定 CSS 高度 |
...
| 顶部入口重复 | 删除自绘标题与菜单入口，采用 Typora 标准原生七菜单 | 原生窗口配置事务目标 PASS；最新原生七菜单与真实安装核验通过 |
...
| 七个真实子菜单 | 使用宿主标准菜单栏的各自子菜单；`framelessWindow=false`，不修改 ASAR 或桥接主进程 | 合成配置安装／重装／恢复、损坏拒绝及回滚 PASS；最新原生七菜单及55秒正常存活验证通过 |
| 快速打开与文件操作 | Ctrl+P 恢复 `440ec3f` 中的文件选择器，鼠标入口在标签条右侧；新文件默认打开并渲染；SCM 提供打开、丢弃、暂存及差异视图打开文件按钮 | 快捷键 renderer 目标 PASS，原生快捷键冲突验证中；文件与 SCM 目标通过；硬件按键冲突未实证 |
...
文件快速打开使用 `440ec3f` 中的文件选择器，Ctrl+P 或标签条右侧入口打开。系统标题栏没有可供工作台挂载的 DOM，不在其内放按钮。最终 UI 与原生集成结果见本节记录；renderer 快捷键目标通过不等于宿主冲突已经排除。
