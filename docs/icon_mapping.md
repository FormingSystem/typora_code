...
R068.1（2026-09-22）：围栏复制使用现有官方Codicons `copy`、成功`check`、失败`warning`；16px字形、24px紧凑动作，圆角/键盘焦点走共享交互。正文外覆盖层不参与代码块行盒。当前反馈单行、明暗/缩放及图片/图表入口回归见[代码块证据](../enhancements/tests/evidence/code_copy_20260922.json)。
