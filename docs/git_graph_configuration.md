...
2026-09-13 用户授权补齐截图中的实际功能，工具栏范围标签／显隐／快捷键、仓库分区和 Git 右键操作按同一固定版本独立实现，使用方式与状态条件见[源代码管理与 Graph 操作](git_scm_actions.md)。新增仓库设置 `history_toolbar_hidden=[]`、`history_shortcuts={}`，分别保存工具栏隐藏动作和绑定；列表／树形仍归既有 SCM 布局记录。前文五个 `scm.graph.*` 专用配置差异保持，不将工具栏功能与其混为一谈。

## R022 提交文本布局补充

2026-09-13，提交标题和作者采用同一区域的行内排版与末端省略；引用不再替代作者，分支描述按固定上游的100px上限裁切。作者0.9em与0.5em间隔取自 IconLabel，当前提交字重600；没有新增用户设置。极多引用仍沿用本产品逐个标签并在容器内裁切，未实现的上游过滤／聚合差异保持原记录。具体职责和验收见[提交标题与作者连续排列](workspace_interaction.md#r022-提交标题与作者连续排列)。
