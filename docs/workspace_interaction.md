...
所有共享菜单固定为勾选、标题、快捷键、子菜单箭头四列；菜单按内容自然宽度并受视口约束，快捷键与箭头不压缩，长标题在有限宽度省略且保留完整可访问名称。尺寸由共同样式持有，调用方只提供条目/状态。保留现有标准与紧凑行高、纵向滚动、定位、启用/取消及焦点返回，不改变菜单业务命令。参考固定VS Code的menu.ts内联样式：标签与keybinding分列，快捷键不折行、菜单按内容布局；本工程用grid实现同一职责。
