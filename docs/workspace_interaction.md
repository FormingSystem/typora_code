...
验证：单元验证显隐/取消/拖动/销毁状态及20/100/1000循环；功能在真实Chromium验证中间透明度、实际鼠标/滚轮/键盘/拖动、明暗/缩放、嵌套区域及第三方边界；原始Typora独立副本验证正式入口接线、真实Explorer滚动区域和文档保持。记录截图与实际alpha，不只以最终透明状态宣称动画完成。R049以前的“无监听/观察器”仅适用于纯样式阶段，本小节替代显隐部分；不增加DOM观察器。

2026-09-20本次交付：全局显隐已实现；完整check、7个关联UI、共享滚动条回归、25项淡出UI、原生7项/20循环、单元20/100/1000通过。候选2026.09.20.3已安装，27资产及head一致、5项保护摘要不变、安装检查OK。原生系统减少动画为true时直接隐藏，真实渐变像素在独立Chromium媒体模拟下验证；不混称物理画面验收。用户窗口尚未重启，未推送。详见[当次证据](../enhancements/tests/evidence/scrollbar_fade_20260920.json)。
...
2026-09-20：齿轮菜单的快捷键被截断并出现横向滚动。共享工厂在带class或快捷键时创建四个单元，标准样式却只有三列；快捷键占用16px箭头列，箭头落入下一行。不能靠单独隐藏设置菜单滚动条掩盖内容丢失。

共享菜单的标题、快捷键、子菜单箭头分列；勾选列按R065.1的条目能力分配。菜单按内容自然宽度并受视口约束，快捷键与箭头不压缩。尺寸由共同样式持有，调用方只提供条目/状态。保留现有标准与紧凑行高、纵向滚动、定位、启用/取消及焦点返回，不改变菜单业务命令。参考固定VS Code的menu.ts内联样式：标签与keybinding分列，快捷键不折行、菜单按内容布局；本工程用grid实现同一职责。

验收复用widgets、preferences与原生稳定性用例，涵盖无class/自定义class/compact、带无快捷键、长标题、子菜单、窄窗口、明暗和100%/125%缩放；检查scrollWidth/clientWidth及实际文字行盒，不能只检查外框。失败/取消不执行命令，销毁清理沿用公共机制。

本次结果：2026.09.20.15通过7组相关UI、完整check和原生237断言，安装29资产一致；旧候选在相同原生用例中失败。正式[证据](../enhancements/tests/evidence/menu_overflow_20260920.json)关联本次测试与安装。标准菜单保留360px最大宽度，紧凑菜单保持原有密度；不是只隐藏滚动条，实际名称/快捷键行盒已核对。


### R065 功能名称完整显示纠正（2026-09-20）

用户再次复现：前版各按钮独立grid按自己的快捷键计宽，在共享容器内仍可能挤压另一行的名称；单行最大宽度不足以代表全菜单共同列宽。替代前版“正常窗口允许省略名称”的约定：共同菜单在入DOM后用实际字体测量所有标签和快捷键的最大宽度，加固定槽/间距/边框，确定共享列宽及菜单宽度，再定位。最大宽度以视口为限；极窄时标签换行，快捷键保留，按钮随内容增高，不出现横向滚动或功能名省略。所有调用方复用，无设置专用补丁。验证同一菜单跨行最长名称/最长快捷键、实际文本Range、中文、明暗、缩放、320px视口及原生宿主；旧测试长名称曾撑大菜单掩盖问题，新增无长名称的实际三项设置菜单。


### R065.1 按条目能力分配勾选位置

2026-09-24：用户反馈编辑菜单和视图菜单中的普通命令左侧都有无意义空白。顶栏、共享菜单以及核心Menu各自无条件创建空状态槽；共享宽度还固定累加该槽。以本节替代此前“所有行固定四列”的约定。

共同状态构造器以`checked`是否为布尔值判定能力，设置`menuitem`/`menuitemcheckbox`及`aria-checked`，只为可勾选项创建16px官方check图标槽。false保留槽，true显示勾；禁用不改变能力。普通命令不建空槽，即使同菜单有勾选项也不缩进。核心Menu已有真实业务图标仍保留，不能把真实图标当空白删除；原生及第三方菜单不在此范围。

共享菜单按各行实际状态槽加标签宽度求最大值，快捷键列共用宽度并在右侧对齐；标准/紧凑共用可选列规则。顶栏及核心Menu继续各自布局与生命周期，复用同一能力判定，不按文字或当前是否已勾选猜测。Esc、外点、子菜单返回、禁用和命令调用不变，不添加轮询/刷新。

上游依据：固定VS Code 1.136.2 `88e44fa0e00b08f7758b4f6d05632e4fd5e4df6f` 的`src/vs/base/browser/ui/menu/menu.ts`，`updateChecked`按undefined区分普通和勾选，`render`分离check/label/keybinding，图标16px。上游本身也预留勾选区域；本次逐行取消普通项空位是用户明确覆盖该布局的产品约定，不宣称与上游像素一致。保留已验证的顶栏8px行内边距、核心26px勾选列、共享16px槽及6px间距，不引入新尺寸。

验收覆盖三类工厂、全普通与混合项、true/false/禁用、真实图标、嵌套菜单、长名称/快捷键、明暗/100%与125%/窄视口；检查实际文字行盒、右侧对齐和非悬停状态。原始Typora用实际顶栏菜单验证，候选同时通过安装与两类卸载重装；不以源码DOM断言代替宿主验收。

R065.1本次验收：2026.09.24.4，5组UI与原始Typora199项通过，同候选安装/两类卸载重装及本机检查通过；用户保存重启加载，Win10/物理键鼠未复验。详见[证据](../enhancements/tests/evidence/menu_slots_20260924.json)。

## H001.2 顶栏移动与窗口尺寸

2026-09-20用户反馈拖动顶栏空白处会变为固定大小。目标是普通窗口移动只改变位置，最大化拖下恢复该窗口原有还原尺寸；尺寸与最大化状态由宿主窗口统一管理，不在菜单或文件拖放中另设默认宽高。保留现有布局、系统贴靠、双击和窗口按钮。

当前顶栏使用原生`app-region:drag`，未发现前端设置固定窗口大小的代码；根因与可用宿主接口待实测，不先添加定时器把尺寸强改回去。修复须覆盖普通/最大化/还原、用户主动缩放、关闭重开及多窗口隔离；失败保留原窗口和文档，不重启用户窗口。原生独立副本检查窗口外框，区分真实系统移动与renderer模拟。

本次核对：用户重新打开后恢复正常。独立Typora 1.14.10的两档普通窗口通过系统移动消息改变位置，960×640及1130×720宽高保持，最大化后还原同样保持；追加非客户区消息没有完成真实最大化拖下，不能冒称已验收。当前不改产品尺寸逻辑，不把再次打开正常视为已定位修复。若复发，记录同一窗口的外框/还原矩形、最大化/贴靠/DPI及运行版本，再关联实际调用。证据见`enhancements/tests/evidence/window_move_20260920.json`。

## R066 弹窗关闭与操作区布局

2026-09-20用户要求补齐右上角关闭入口，并将确定、取消等底部操作右对齐。根因是workspace_dialog只有h3/content/footer，footer默认左对齐；Git丢弃与大纲另有局部基础样式。共享工厂统一拥有标题栏、官方Codicons close、底部操作区及生命周期；领域保留内容、按钮顺序、启用条件与业务回调，不按按钮文字猜测确认/取消，也不改变系统原生对话框、快速选择器、菜单和独立媒体查看器的布局。

右上角×和底部关闭/取消调用同一close，沿用R032/R033的Esc、外点、焦点归还及单次清理语义。检查更新关闭仍取消检查，安装进度关闭仅隐藏视图；待确认设置关闭不保存。已有领域明确禁用/进行中的业务行为不由新增入口重写。默认焦点优先原有内容/底部控件，避免新增×抢走输入；×可通过Tab、Enter/Space访问。可见控件动态扫描继续排除禁用和隐藏项，嵌套层只关闭自身。

依据固定VS Code 1.137.0提交645f29c的dialog.ts关闭动作映射cancelId、dialog.css右上工具区及按钮组flex-end规则；本产品保留现有13px工作台字体、标题字号、760px最大宽度与原按钮顺序。关闭按钮采用20px工具行命中盒、16px官方字形和共享4px交互圆角；标题可换行且不覆盖×。标题/底部为固定区域，正文独立滚动；底部按钮自然换行、每行右对齐，不隐藏名称。共享样式归workspace_widgets，移除Git/大纲的重复底部基础定义；领域仅保留实际内容与宽度差异。

验证共同工厂以及设置、Git确认、文件操作、更新和插件调用方，覆盖真实关闭点击、单次回调、Esc/Tab焦点、长标题、多按钮、动态插入、禁用、窄窗口、主题与缩放；原始Typora正式构建验证宿主样式不使标题/图标/底部错位。原生系统选择框由宿主维护，不宣称本次重排它们。完成构建、检查、本机安装后记录用户需正常重启及发布结果。

验收证据见[弹窗验证记录](../enhancements/tests/evidence/dialog_controls_20260920.json)。2026-09-20用户后续明确本次先本机检查效果、暂不推送；下一次启动Typora加载已安装候选。更新包复用反馈由用户撤回，本次不改下载缓存逻辑。


2026-09-22 R071.1：`primary`角色集中读取VS Code button.background/foreground/hoverBackground；既有提交分裂按钮和空仓初始化共用默认/悬停颜色。几何与分裂圆角继续由领域拥有；禁用不启用悬停、none和正文/第三方编辑器边界不变。新增颜色变量覆写和禁用优先级回归，来源为固定88e44fa版本button.css。

## R008.3 展开浮层的刷新与稳定定位

2026-09-23用户反馈底栏快捷图标展开后在其他电脑持续刷新、抖动。目标是同一菜单打开期间保持节点、焦点和滚动，仅在内容、锚点或视口实际变化时重新定位；保留原生可用性、动作及现有外观，不接管领域业务刷新或以定时轮询替代事件。

共享浮层刷新器拥有帧合并、布局期间自身观察记录的隔离及销毁取消；底栏适配负责原节点及包含块坐标，悬停负责展示/取消和内容会话，原生工具栏负责所属阅读范围。小数CSS像素的重复布局必须收敛，不能由上一次舍入误差反推下一次包含块原点。宿主或异步内容真正改变后仍刷新；关闭时不得重建或重新打开，销毁恢复适配前样式。

实现前复查现有R012/R008.1的宿主节点与定位约束。布局外观沿用固定VS Code菜单/悬停来源及现有尺寸；本次只修复调度与坐标反馈，不新增视觉尺寸。验收以连续帧几何、样式写入次数、节点身份/焦点/滚动保留和真实动作计数为依据，覆盖100%/120%/125%/150%、小数包含块、长短内容、主题、动态更新、关闭与清理；原始Typora与隐藏Electron证据分开，未到场的其他电脑不宣称通过。

当前实现：`workspace_popup_refresh`按帧合并观察器和显式定位请求，比较属性批次前后值以及布局提交后的尺寸，消费本观察器的自身布局记录，不屏蔽宿主观察器。底栏从零CSS坐标测量包含块原点，目标差异小于1/32 CSS像素时不再写位置，避免序列化精度反复累积。共享悬停更新尺寸时保留滚动位置，异步长短内容通过局部内容观察更新；关闭重置观察器、销毁取消待执行帧。原生阅读工具栏复用同一生命周期。顶部菜单已有宽度去重，共享操作菜单按打开快照构建、无周期刷新；不为这些入口再增加第二套业务状态。

来源复核：固定VS Code 88e44fa的`hoverWidget.ts`/`hoverWidget.css`及既有菜单设计，尺寸与风格不变；Typora1.14.10原生文件菜单开关、最近目录异步读取、字数和语言点击仍沿原委托事件。数值容差只用于抑制布局舍入，不是新的界面尺寸。验证及安装边界见[本次证据](../enhancements/tests/evidence/popup_refresh_20260923.json)。
