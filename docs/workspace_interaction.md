...
2026-09-24早先按行取消普通项空位，混合菜单因此出现文字起点不一致。用户随后明确不要紧凑版，所有下拉菜单以VS Code为准；本节旧排版约定由R065.2替代，旧交付证据保留作历史记录。

## R065.2 下拉菜单统一列对齐

问题与目标：格式等菜单混合普通、可勾选与子菜单项，逐行有无标记改变文字起点，快捷键又随箭头挤动。整张菜单必须统一列，状态只改变标记可见性。2026-09-24用户明确取消紧凑例外，纯命令菜单也采用同样的左侧空间。

来源：本地VS Code 68070681e87284e2f22728f15fe3f3651fbf932b的src/vs/base/browser/ui/menu/menu.ts，render构建check/label/keybinding，样式24px行、13px继承字体、4px行侧边距、2em标记宽度与标签/快捷键内距、绝对定位check、快捷键右对齐且常态0.7透明度。采用统一2em文字起点，16px官方图标，菜单按实际文字测量公共快捷键列；每层独立测量，不观察正文或增加全局刷新。

职责与边界：workspace_menu_item管理条目能力及菜单列测量，顶栏/共享上下文/核心Menu适配各自DOM，不各自按勾选类型选布局。普通项保持menuitem，无伪造aria-checked；可勾选false/true保持menuitemcheckbox和真实状态。打开时取快照测量，关闭/取消沿原焦点与销毁路径；不改变业务命令、宿主选区、顶部栏高度、菜单类别和原生第三方菜单。快捷键无内容时也不推挤箭头，长名称保持原菜单滚动/裁剪边界。

验收：八类真实顶栏尤其格式/段落，普通/混合/禁用/有子菜单和真实图标，检查文字实际行盒起点、标记中心、快捷键右边缘和箭头；三主题/100%与125%、窄窗、键盘导航/取消和选区恢复。相关UI和原生宿主使用本次候选，同候选隔离卸载重装、本机只读卸载预检与安装分别记录。
...
2026-09-24 R020.1/R034.8：选中、焦点、展开和活动内容来源是四个独立状态。资源管理器、文件夹选择器、Git提交与文件使用同一选择模型及整行绘制。共享模型管理稳定键的选择集合、焦点与可见行投影，领域负责键、打开/展开命令和多选策略；不把业务服务合并进UI。虚拟行按键恢复，切库清除、销毁解绑，无定时器或正文监听。

普通点击单选；已有Ctrl/Shift多选保留，取消选择不能被焦点重新选回。Git树中点击提交选择提交，点击文件将选择移到文件；展开不自动选择父行。来源投影只在活动身份改变或用户明确定位时接管选择，重复刷新不能覆盖手动选择。异步文件读取只填充行，不能反向切换选择或正文。

按固定6807068的2026-light.json/2026-dark.json和listWidget/listService的状态角色，统一默认、悬停、活动选择、非活动选择和键盘焦点。亮色选择分别为#00000025/#DADADA99、悬停#00000014；暗色为#FFFFFF22/#2C2D2E、悬停#FFFFFF14。保留alpha，不能用元素opacity使文字和图标一起变淡，不重复叠加父子背景。列表底板配套#FAFAFD/#191A1B；这替代R074对列表区域的Modern不透明映射；其他功能区域配色同步按颜色设计更新，正文主题保持。尺寸与缩进由视图保留，背景覆盖整行。Git拓扑只读取公共状态实现自己的圈与线。

验收：资源管理器及选择器、SCM提交/文件、中央Git文件的同状态颜色与透明度；鼠标/键盘、失焦、单/多选、展开与选择独立、宽窄/长名称/明暗、虚拟重建及销毁。实际绘制与原生宿主另验，不用旧截图代替当前构建。


### R065.2 菜单文字裁切补充（2026-09-25）

Night菜单的g等下伸字形被裁，原因是本地grid标签同时使用line-height:1和overflow:hidden。固定上游6807068的menu.ts使用24px行与flex、文字行高1但未给文字标签增加此裁切。保持共享24px行和列宽，本地文字/快捷键行盒填满24px以容纳真实字形，不扩大菜单或修改正文。验证中文、Night、长Light/Dark名称在明暗及100%/125%缩放中的文字Range位于标签盒内；菜单选择、快捷键与图标列不变。
