...
R069.8（2026-09-23）修正上述排除规则的实际事件边界：沿完整composedPath排除独立预览，不从影子树内#write误判主正文。正文原窗口缩放、菜单及快捷键的既有含义保持；预览滚轮只改变当前阅读器比例，不执行这些窗口命令。验收必须从真实影子节点派发并同时绑定主监听，详见[链接预览设计](link_preview.md#r0698-预览事件与链接手势隔离)。
