...
用户指出Git明细未落实为逐行列表。现有规则仍允许只写标题，与最新要求不一致；已更新AGENTS及开发skill，增加只读格式检查入口，提交前检查消息文件并在提交后读取实际正文。

三个本地分支共86条历史中，64条补具体明细、3条转换原段落、19条保留原列表。每条tree、标题、作者/提交者和日期及拓扑保持，完整bundle已隔离恢复验证，格式与Git对象检查通过，27项产品资产不变。只更新本地分支，remote-tracking与远端保持；无重新构建或安装。备份、旧OID追溯与发布边界见[历史处理](git_commit_messages.md)。
