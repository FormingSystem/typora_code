...
本章命令均从独立 Typora Code 仓库根目录执行。当前部署使用官方社区插件入口；Windows 隔离安装、重装、卸载与失败回滚回归已通过，Linux Bash 文件事务已验证，原生 Linux 入口仍待实机验证。
...
1. 构建检查按共享 `bundle_markers.txt` 验证能力入口，安装时按 `SHA256SUMS` 校验本插件包、官方 loader/core 与终端模块，核对随运行资源部署的图标许可；Windows 另校验官方 Node 下载包及可执行文件；
2. 解析并验证 Typora 安装根；
3. 备份当前主题、`resources/window.html`、插件启用设置和本次将覆盖的插件文件；
...
插件核心固定为 Typora Community Plugin `2.10.15`，原始核心、CSS 和三个语言包随仓库分发。`window.html` 只加载官方 `typora://app/userData/plugins/loader.js` 模块入口，loader 根据 `plugins/loader.json` 导入 `plugins/2.10.15/core.js`。本插件由核心加载，部署在 `plugins/plugins/forming_system.linux_note_enhancements/`，包含 `main.js`、`manifest.json` 和 `style.css`。安装器校验各自摘要清单，仅合并 `plugins/settings/plugins.json` 中本插件的启用项；其他设置与插件文件保持原值。Linux JSON 合并需要 Python 3，Windows 使用 PowerShell；安装器不执行上游安装脚本或创建仓库符号链接。来源、许可证与摘要见 [工作区依赖说明](./enhancements/vendor/typora_workspace/NOTICE.md)。

工作区按钮、搜索与 Git 菜单使用 52 个官方 Codicons SVG，固定源码与摘要保存在仓库，图标 CC BY 4.0 与代码 MIT 许可随部署保留。安装不读取本机 VS Code 的文件、字体、配置或扩展；普通用户直接使用预构建资源。维护方法见 [开发者构建](./enhancements/README.md#1.2_开发者构建)。
...
回退前还会在备份目录保存当前 `window.html` 的安全副本。回退将新增的主题、本插件与终端文件改名停用，原先存在的文件恢复备份；只还原本插件的启用设置，保留其他后来改动的设置。若其他社区插件仍安装，保留共享 loader/core 与官方入口，使它们继续可用；否则按清单恢复共享核心。安装失败则回滚到安装前状态。当前恢复清单使用 schema v2，不读取旧 direct bundle 的备份格式。
...
检查通过时会报告平台、已验证的 Typora 根目录、唯一脚本入口数量、主题 SHA-256、插件 main.js SHA-256 和 `status: OK`。检查会分别比较已安装主题、bundle 与当前仓库文件是否一致，并校验全部插件资产；旧主题、旧 bundle 或缺失、损坏的插件文件不能仅凭入口还在就通过。普通安装也会在复制后校验主题、bundle 和插件资产。持续集成或其他非交互环境应增加 `-non_interactive` 或 `--non-interactive`，防止脚本等待终端输入。
...
- 按 `Ctrl + Shift + F` 搜索，确认只有统一搜索框，检查文件图标、目录、Git 状态和高亮，悬停查看行列；默认单击在编辑区预览打开，双击或 Enter 保持打开并核对原生 Markdown 或其他源码的起止选区。分别验证大小写、全字、正则、包含／排除、忽略规则、仅已打开及仅源代码管理更改文件。替换先看双栏预览，预览后改动磁盘文件或保留未保存正文时应拒绝执行；移除结果只影响本轮搜索。
- 选中文字后 Ctrl／Cmd 加左键，确认文字进入同一个搜索面板；分别准备仅一个和多个命中文件，都先列结果、不自动打开。默认单击文件或片段在编辑区预览打开；显式开启下方阅读预览时更新下方区域并保持中央位置；双击片段保持打开并精确选中内容，双击文件使用当前命中或首条。检查 Markdown 标题、粗体段落、围栏同行第二处命中、Alt 前后导航及源码草稿保护；预览支持收放、独立滚动、上下分界线、滑块和 Ctrl／Cmd 加滚轮的 50%～150% 缩放及比例持久化。
...
2026-09-06 已有 Windows Typora `1.14.9` 基线验证覆盖默认单组标签、跨文件增加标签、左右／上下分栏、点击预览切入编辑、文内与跨文件后退／前进及滚动恢复，测试 Markdown 的文件摘要未变化。代码按钮与颜色还通过隐藏 Chromium 的真实输入回归。PowerShell 5.1 的隔离安装、重复安装、校验、恢复和失败回滚通过；Bash 插件资产事务和语法检查通过，未替代原生 Linux／UCRT64 验收。

全量 `npm run check`、TypeScript 与最终安装一致性检查通过。Windows Typora 最终 `workspace` 实窗通过 75 项，包含单行顶栏、全局状态、搜索下方直接缩放与渲染预览；Git 实窗通过 66 项，重命名与 Markdown 精确定位各通过 16 项。预览隐藏 Electron 通过 31 项，包含隔离的本机 Mermaid 图表库；加载真实原生 CSS 的搜索布局和 Explorer 各通过 20 项。最终完整窗口画面已检查，具体结果及入口集中在 [开发者构建与验证](./enhancements/README.md#1.2_开发者构建)。
...
选中文字后 Ctrl／Cmd 加鼠标左键进入同一个搜索面板；无论多少命中都先列结果，默认单击在编辑区预览打开，双击或 Enter 保持打开并按行列选中内容；下方只读阅读预览在搜索视图选项中显式开启。Markdown 显示主题标题、列表、表格与围栏高亮，其他语言显示源码；Mermaid 在独立 iframe 中使用 Typora 随附的本地库，不改中央图表配置。预览可以收放，默认采用正文字号的 80%，通过 50%～150% 滑块或 Ctrl／Cmd 加滚轮调整并保存，不改变中央文档。侧预览上限为 2 MiB，文档 HTML 经过净化，不执行文档脚本或加载媒体。它查找文本出现位置，不承担语言服务器的定义解析；完整操作见 [选中文字的跳转预览](./enhancements/README.md#1.4.6_选中文字的跳转预览)。
...
Markdown 使用一个活动的 Typora 原生编辑器，其余分栏显示预览；点击预览正文会交换编辑器所在分栏。普通源码标签使用独立可编辑缓冲区和格式选择，切换、跨组拖动与右键分栏移动保留草稿，保存前核对磁盘内容；后台标签关闭和窗口关闭都处理未保存修改，原生 Markdown 取消关闭后仍继续保护源码。该 Markdown 的源码标签尚未保存时，先处理草稿再打开渲染；精确跳转会核对磁盘、当前内存和最终选区，并保留既有标签与阅读历史。Git 历史、差异与搜索侧预览只读。VS Code 语言服务器和扩展宿主不在本实现范围内，切换 Markdown 仍遵循 Typora 的保存确认。完整操作和历史边界见 [标签页、分栏与阅读历史](./enhancements/README.md#1.4_标签页、分栏与阅读历史)。
...
Windows／Linux 的新窗口同时启用 Typora Unibody 样式，将应用图标、中文主菜单、文档标题和原生窗口按钮合为一行，随主题变化。现有窗口的系统边框不会被强制替换；保存文档并正常重启后生效。菜单、标签与正文分别占用自己的区域，不再叠加系统标题行与独立菜单行。
