...
1. 按共享 `bundle_markers.txt` 验证代码与 Mermaid、阅读导航、全文件浏览、源码查看、工作区搜索、活动栏、Git Graph／差异和终端等入口，并按 `SHA256SUMS` 校验社区插件核心与终端模块，核对随运行资源部署的图标许可；Windows 另校验官方 Node 下载包及可执行文件；
2. 解析并验证 Typora 安装根；
...
工作区按钮、搜索与 Git 菜单使用 42 个官方 Codicons SVG，固定源码与摘要保存在仓库，图标 CC BY 4.0 与代码 MIT 许可随部署保留。安装不读取本机 VS Code 的文件、字体、配置或扩展；普通用户直接使用预构建资源。维护方法见 [开发者构建](./enhancements/README.md#1.2_开发者构建)。
...
- 按 `Ctrl + Shift + F` 搜索，检查文件分组、目录、行号与高亮，点击结果后核对源码起止选区；分别验证大小写、全字、正则、包含／排除、忽略规则和仅已打开。替换先看双栏预览，预览后改动磁盘文件或保留未保存正文时应拒绝执行。
...
- 打开较长差异，检查两侧各 8px 滚动条、30px 红绿概览和修改版缩略图；点击红／绿标记后两栏同步定位。切换浅色／深色主题，现有终端应更新背景、文字、光标和 ANSI 配色，Shell 与输出保持。
...
本次新增全文件、源码与搜索能力已有 `-suite browser` 的 13 项 Typora `1.14.9` 实窗检查通过，阅读套件通过 18 项（首次窗口 15 项、重新开窗 3 项），Git 与终端原生套件分别通过 66 项、13 项；活动栏、侧栏底部布局、原生红绿差异概览及终端主题另有独立隐藏 Electron 回归。搜索后台 8 组、Worker 隔离 4 组回归通过，验证取消与单文件 2 秒超时可终止正则匹配并阻止不完整替换。各入口、通过范围及本轮最终安装状态集中在 [开发者构建与验证](./enhancements/README.md#1.2_开发者构建)，不能用旧安装基线代替当前 bundle 的校验。
...
文件树显示全部文件和隐藏目录，目录按需读取；普通源码使用占满编辑组的单栏只读 Monaco，Markdown 正常渲染。特殊文件名、复合后缀、解释器识别及二进制边界见 [全部文件与语言识别](./enhancements/README.md#1.4.3_全部文件与语言识别)。搜索侧栏按文件展示路径、行号与高亮，点击后精确选择源码；大小写、全字、正则、包含／排除、忽略规则、仅已打开以及确认替换见 [工作区搜索与替换](./enhancements/README.md#1.4.4_工作区搜索与替换)。
...
主侧栏下半区直接展示 **提交图**，点击提交展开修改文件，上下分区可拖动调整；收起后标题仍可见，不被原生底部工具条覆盖。完整提交图、文件时间线和 Monaco 左右差异在原有编辑标签栏中打开，支持源码高亮、行对齐、行内改动、查找和改动导航；差异两栏保留各自 8px 滚动条并纵向同步，30px 原生红绿概览和修改版源码缩略图均可点击定位。普通文档也提供右侧缩略图，长文可点击或拖动跳转。分支、贮藏、合并、变基、远端同步与评审继续在这些入口操作。完整收集与对应入口见 [Git Graph 功能对照](./enhancements/git_graph_features.md#第1章_Git_Graph功能对照与操作说明)。操作与查询边界见 [Git Graph 提交关系图](./enhancements/README.md#1.5_Git_Graph提交关系图)。

Markdown 使用一个活动的 Typora 原生编辑器，其余分栏显示预览；点击预览正文会交换编辑器所在分栏。普通源码标签和 Git 差异只读，多份独立未保存缓冲区、VS Code 语言服务器等能力不在本实现范围内，切换 Markdown 仍遵循 Typora 的保存确认。完整操作、插件管理和历史边界见 [标签页、分栏与阅读历史](./enhancements/README.md#1.4_标签页、分栏与阅读历史)。
