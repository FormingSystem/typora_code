...
1. 按共享 `bundle_markers.txt` 验证代码与 Mermaid、阅读导航、全文件浏览、源码查看、统一搜索与下方预览、活动栏、Git Graph／差异和终端等入口，并按 `SHA256SUMS` 校验社区插件核心与终端模块，核对随运行资源部署的图标许可；Windows 另校验官方 Node 下载包及可执行文件；
2. 解析并验证 Typora 安装根；
...
工作区按钮、搜索与 Git 菜单使用 45 个官方 Codicons SVG，固定源码与摘要保存在仓库，图标 CC BY 4.0 与代码 MIT 许可随部署保留。安装不读取本机 VS Code 的文件、字体、配置或扩展；普通用户直接使用预构建资源。维护方法见 [开发者构建](./enhancements/README.md#1.2_开发者构建)。
...
- 按 `Ctrl + Shift + F` 搜索，确认只有统一搜索框，检查文件图标、目录、Git 状态和高亮，悬停查看行列；单击只更新预览，双击核对原生 Markdown 或其他源码的起止选区。分别验证大小写、全字、正则、包含／排除、忽略规则、仅已打开及仅源代码管理更改文件。替换先看双栏预览，预览后改动磁盘文件或保留未保存正文时应拒绝执行；移除结果只影响本轮搜索。
- 选中文字后 Ctrl／Cmd 加左键，确认文字进入同一个搜索面板；分别准备仅一个和多个命中文件，都先列结果、不自动打开。单击文件或片段只更新下方预览，中央位置保持；双击片段精确选中内容，双击文件使用当前命中或首条。检查 Markdown 标题、粗体段落、围栏同行第二处命中、Alt 前后导航及源码草稿保护；预览支持收放、独立滚动、上下分界线、滑块和 Ctrl／Cmd 加滚轮的 50%～150% 缩放及比例持久化。
...
选中文字后 Ctrl／Cmd 加鼠标左键进入同一个搜索面板；无论多少命中都先列结果，单击只更新下方只读预览，双击才按行列打开并选中内容。Markdown 显示主题标题、列表、表格与围栏高亮，其他语言显示源码；Mermaid 在独立 iframe 中使用 Typora 随附的本地库，不改中央图表配置。预览可以收放，默认采用正文字号的 80%，通过 50%～150% 滑块或 Ctrl／Cmd 加滚轮调整并保存，不改变中央文档。侧预览上限为 2 MiB，文档 HTML 经过净化，不执行文档脚本或加载媒体。它查找文本出现位置，不承担语言服务器的定义解析；完整操作见 [选中文字的跳转预览](./enhancements/README.md#1.4.6_选中文字的跳转预览)。
...
资源管理器的文件和文件夹可通过 F2、选中后双击名称、右键 **重命名** 修改名称。输入框按 Enter 确认、Esc 取消；打开标签及草稿的保存路径随改名更新，搜索列表与阅读历史也同步更新。名称冲突会提示，不将文件名输入当作目录移动命令。
