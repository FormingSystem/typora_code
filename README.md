...
| SSH远程 | 左侧远程资源管理器连接Linux主机，浏览目录、新建文件与文件夹、远程文本读写、Markdown阅读预览、项目终端与Git状态；[使用与边界](docs/remote_ssh.md) |
| 搜索 | 工作区内容搜索；单击侧栏预览，双击或 Enter 打开；重复单击返回命中位置 |
| 链接预览 | 搜索侧栏打开时选中Markdown链接；右键左右/上下分屏只读预览，本地目标可打开源文件编辑。网页为联网只读快照，见[使用边界](docs/link_preview.md) |
