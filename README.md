...
分屏与拖动已修复删除/合并后的比例错位，分隔线只调整相邻两栏；标签重排保留当前编辑器，拖动停止滚动后不再空转动画。20／100／1000轮布局和跨窗口协议压力、20次原生建窗合并的结果及操作边界见[拖动与多窗口](docs/drag_and_windows.md)。
