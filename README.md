...
当前版本为 **2026.09.20.20**，各版本修复公告以[发行记录](enhancements/release.json)为准。手动检查更新立即显示状态，支持取消并明确提示检查结果。切换目录会隔离旧工作区，并按原生恢复配置恢复目标目录上次打开的文件；空编辑区不再显示假文件。Git 提交详情支持 Markdown 列表与代码显示。底栏图标复用共享居中规则，滚动条离开后渐隐；社区插件在工作台显示后加载。资源管理器、时间线和Git一级分区共用留白与整行悬停背景；共享菜单完整显示名称及快捷键，极窄窗口换行。原生图标启动后一次呈现完整工作台，不显示加载提示覆盖层；侧栏功能切换保持展开，详见[启动与切换稳定性](docs/startup_stability.md)。
...
分屏与拖动已修复删除/合并后的比例错位，分隔线只调整相邻两栏；标签重排保留当前编辑器，拖动停止滚动后不再空转动画。拖出仅接收指定文件，不恢复原目录整组标签；合并不需要额外确认，宿主短暂切换会自动等待，失败保留文档并轻提示。20／100／1000轮协议压力、源码及已保存/未保存Markdown各20次原生往返的结果及操作边界见[拖动与多窗口](docs/drag_and_windows.md)。
