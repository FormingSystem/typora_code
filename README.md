...
| SSH远程 | 左侧远程资源管理器连接Linux主机，浏览目录、新建文件与文件夹、远程文本读写、Markdown阅读预览、默认远程终端与Git状态；[使用与边界](docs/remote_ssh.md) |
...
**2026.09.22.12** 修复SSH连接后的默认终端：活动栏、菜单、新建和快捷键进入当前远端目录；拆分与重启保持会话归属，断线不回退本地，本地Shell需明确选择。完整远程资源管理、搜索、SCM和原生Markdown编辑仍在接入，不能将本次终端修复视为这些功能已完成。
