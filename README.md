...
| 链接预览 | 选中Markdown链接在左下独立预览，右上关闭；顶部、右边和右上角可调整高宽，收起功能栏仍保留预览，正文不预留整列空白；本地预览顶栏提供50%—150%缩放滑条和百分比；预览内链接直接跳转，Alt+左右恢复独立历史与阅读位置，新选中链接重新开始；右键左右/上下分屏只读预览，本地目标可打开源文件编辑。网页在隔离页面加载，见[使用边界](docs/link_preview.md) |
