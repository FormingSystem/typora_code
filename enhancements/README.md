...
点击左侧工具栏底部的 **分支图标**、状态栏 **Git Graph**，或在社区命令面板执行 **Git Graph：查看提交关系图**。图在当前编辑区新增标签，可继续分栏；当前文档用于发现 Git 仓库，切回正文保留阅读位置。

| 阅读或操作目标 | 入口 |
| --- | --- |
| 查看提交、引用、stash 与未提交改动 | 提交图；分支框支持单选、多选与 glob |
| 比较任意两个版本 | 点击第一条，再 Ctrl / Cmd 点击第二条；也可选择工作区 |
| 查看差异、历史文件与路径 | 变更文件及右键菜单；支持统一补丁和双栏文本 |
| 创建分支、合并、变基、标签、stash 等 | 右键相应节点或引用，预览参数后执行 |
| Fetch、Pull、Push 与远端 URL 管理 | 工具栏 Fetch，操作 → 远端配置 |
| 暂存、取消暂存、提交与冲突恢复 | 未提交改动和操作菜单 |
