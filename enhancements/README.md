...
Markdown正文及历史阅读中已显示的图片，在图片右上角提供“全屏查看”，小图显示同一全屏图标。也可直接双击图片打开大图，单击仍用于选中图片。打开后可放大、缩小、拖动、适应宽度／屏幕或恢复100%；Ctrl+滚轮缩放、Esc退出。图片默认完整适应屏幕，小图保持自然大小。Mermaid与图片共用查看器。

`reading_image_viewer.ts`只在正文外定位按钮，不写入Markdown；`reading_media_viewer.ts`统一管理副本、缩放、焦点、主题和关闭。图片格式及来源遵循现有阅读器，搜索预览仍不加载图片。设计和回归见[图片与图表放大查看](../docs/reading_media_viewer.md)。
