...
Markdown正文及历史阅读中已显示的图片，鼠标悬停时在图像外侧右上方显示“全屏查看”，小图显示同一全屏图标。按钮覆盖在预留的工具行中，移出后隐藏；键盘聚焦时也显示。工具行随正文滚动，悬停不引起排版移动，按钮不会覆盖图片内容；图片与Mermaid采用相同的入口布局和样式。也可直接双击图片打开大图，单击仍用于选中图片。打开后可放大、缩小、拖动、适应宽度／屏幕或恢复100%；Ctrl+滚轮缩放、Esc退出并恢复打开前的光标／选区，可直接继续编辑；从键盘入口打开则返回入口。图片默认完整适应屏幕，小图保持自然大小。Mermaid与图片共用查看器。

`reading_image_viewer.ts`负责图片来源，`reading_media_entry.ts`与`.css`统一管理图片和Mermaid的入口；按钮保存在正文外，流内空占位不写入Markdown。`reading_media_viewer.ts`统一管理副本、缩放、焦点、主题和关闭。图片格式及来源遵循现有阅读器，搜索预览仍不加载图片。设计和回归见[图片与图表放大查看](../docs/reading_media_viewer.md)。
