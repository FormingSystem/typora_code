...
Markdown正文及历史阅读中已显示的图片，在图片上方的独立工具行提供“全屏查看”，小图显示同一全屏图标。工具行随正文滚动，按钮不会覆盖图片内容；图片与Mermaid采用相同的入口布局和样式。也可直接双击图片打开大图，单击仍用于选中图片。打开后可放大、缩小、拖动、适应宽度／屏幕或恢复100%；Ctrl+滚轮缩放、Esc退出。图片默认完整适应屏幕，小图保持自然大小。Mermaid与图片共用查看器。
