...
- 手动搜索和选中文字后 Ctrl／Cmd 加鼠标左键共用搜索侧栏；单击更新下方 Markdown／源码预览，双击才打开目标，支持预览收放、缩放和上下分区拖动；
- 顶栏按 VS Code 的单行布局提供各自独立的文件、编辑、段落、格式、视图、主题和帮助菜单；居中搜索框用 `Ctrl + P` 按文件名快速打开工作区文件，左侧标准箭头用于后退和前进；
...
| `test_workspace_explorer.cjs` | 隐藏 Electron：全文件、隐藏目录、按需读取、定位及 F2／双击／右键重命名 |
...
| `test_git_graph_actions_settings_i18n.mjs` | 平台中立：639 键中英词典、语言选择、操作／设置元数据和校验消息 |
...
2026-09-09 已通过 `npm run build`、完整 `npm run check` 和 **25／25** 项 `npm run check:ui` 隐藏 Electron 回归。既有 Windows Typora `1.14.9` 非交互安装曾由 `check_configuration_windows.ps1` 复核为单一扩展入口、5 个工作区资源和 20 个终端资源，状态为 `OK`。当前套件覆盖经典 Git Graph 的 40px 单行工具栏、30px 五列表头、24px 提交行和行内 50%／50% 详情，统一搜索与下方预览的单击／双击语义，含空格、`#`、`%`、方括号及复合后缀的路径，资源改名、逐标签未保存确认、空编辑组、VS Code 快捷键、侧栏收放阈值、大纲随阅读位置同步，以及缩略图原子换帧和非相关侧栏切换零重绘。
...
资源管理器中，选中文件或文件夹后双击名称、按 **F2**，或者右键选择 **重命名**，可直接编辑名称；**Enter** 确认，**Esc** 取消。只允许修改当前父目录下的名称，不将名称输入当作移动路径；同名目标、非法名称、工作区根目录和符号链接路径会明确拒绝。目录改名同步其中已打开标签的路径，保留源码草稿、撤销记录和阅读位置；搜索列表重新刷新。磁盘内容同时变化时保留草稿并提示核对，不能将未保存内容偷偷替换为磁盘版本。
...
结果按文件分组，显示文件图标、名称、相对目录、匹配数、Git 状态与高亮片段；行列位置放在悬停提示和可访问标签中，不再单独占一列。每条结果同时保留结构化绝对路径、相对路径和精确起止行列，显示名称只用于界面呈现，不再参与文件解析。文件分组和目录可折叠，可切换列表／树形显示，按路径／结果数排序，也可通过结果统计旁的 **在编辑器中打开** 展示完整结果。单击文件或片段仅更新下方预览；双击才按结构化路径和起止行列打开文件并选中精确匹配，Markdown 使用原生渲染。右键可在右侧打开、复制匹配行或路径。文件组悬停时可 **从结果中移除**，它只更新本轮结果，不删除磁盘文件。
...
| 操作 | 结果 |
| --- | --- |
| 单击文件或匹配片段 | 只更新面板下方预览，中央文档、光标和阅读位置保持 |
...
面板复用工作区搜索的包含／排除 glob、区分大小写、全字匹配、正则及忽略规则；文件范围与资源上限见 [工作区搜索与替换](#1.4.4_工作区搜索与替换)。改变查找范围后重新显示结果，文件类型继续使用同一套映射。单击预览读取文件，双击才执行打开动作；已有同名文本的不同文件都作为候选展示。
...
Git Graph 界面文案通过 639 个受类型约束的键成对提供简体中文和英文；根据 Typora／插件语言选择界面语言，面板、源代码管理、提示、菜单、设置、确认框与错误消息一起切换。分支名、提交号、路径、Git 参数和用户输入保持原值，不进行翻译。
