...
搜索侧栏、下方 Markdown／源码预览与选中文字入口一起构建进增强 bundle，复用语言识别、搜索和 Monaco 资源；安装、功能标记检查、备份和回退使用同一组入口。用户的预览缩放设置保存在本地，更新运行文件时保留。
...
| `test_workspace_files_search.cjs` | 隐藏 Electron：源码查看、语言映射、搜索分组、精确选区与搜索布局 |
...
| `test_workspace_outline.cjs` | 隐藏 Electron：大纲外围留白、原生过滤状态清理、层级缩进及标题操作 |
...
| `test_workspace_titlebar.cjs` | 隐藏 Electron：VS Code 标题栏尺寸、标准图标、独立菜单、最近记录、界面字体隔离、键盘与窄窗口布局 |
...
2026-09-06 全量 `npm run check`、TypeScript 与安装一致性检查通过。Windows Typora `1.14.9` 最终 `workspace` 实窗通过 **75 项**，覆盖单行顶栏、唯一全局状态栏、统一搜索、下方直接缩放、Markdown／Mermaid 预览和源码草稿；完整窗口画面已检查。`rename` 与 `markdown_location` 实窗各通过 **16 项**，Git 实窗通过 **66 项**。正文／源码预览隐藏 Electron 通过 **31 项**，使用本机 Mermaid `11.13.0` 的独立实例；加载真实 Typora CSS 的搜索布局和 Explorer 各通过 **20 项**，Git Diff 全局状态通过 **10 项**。源码编辑与真实关窗另有 17、8 项通过基线。实窗测试只修改临时样例，核对原文档字节与调用工作树索引并移除测试入口；具体断言以各套件报告为准。
...
Windows 与 Linux 顶栏保持一行：应用图标和七个中文菜单位于左侧，后退、前进及工作区文件搜索位于中央，窗口按钮位于右侧。每个菜单只显示自己的命令；点击中央搜索框或按 `Ctrl + P` 打开文件快速列表，按文件名或相对路径模糊筛选。目录扫描包含点开头的隐藏文件和复合后缀文件，但不读取文件正文；`↑` / `↓` 选择，`Enter` 打开，`Esc` 关闭。后退和前进按钮没有对应历史时自动禁用。

顶栏功能按职责拆分：`workspace_titlebar.ts` 只编排位置，`workspace_titlebar_entries.ts` 映射 Typora 命令与最近记录，`workspace_titlebar_menu.ts` 处理菜单层级和键盘交互，`workspace_quick_open.ts` 管理文件快速打开，`workspace_tabs.ts` 管理关闭事务与空编辑组，`workspace_ui_appearance.ts` 管理应用外壳字体。弹窗、右键菜单和基础 DOM 构造统一由 `workspace_widgets.ts` 提供，搜索、资源管理、终端和 Git Graph 不再跨域依赖 Git 私有工具模块。各功能通过工作区文件宿主组合，不互相读取内部状态；关闭全部标签时逐个执行未保存确认，最后隐藏宿主为维持树结构生成的空标签。

“视图 → 界面字体…”可以独立设置菜单、标签、侧栏控件和弹窗的字体与字号。选择器不匹配 `#write`、Markdown 预览正文或 Monaco 编辑器，Markdown 继续使用 Typora 当前主题，源码编辑器继续使用文件语言对应的编辑器字体。
...
| 视图 → 界面字体… | 只调整应用外壳字体和字号，不改变 Markdown 正文 |
...
普通打开 `.md`、`.markdown`、`.mdown`、`.mkdn`、`.mkd` 进入 Typora 的 Markdown 渲染与编辑；双击搜索结果也使用该视图，并按行列定位及选中命中内容。其他文本在独立的 **单栏 Monaco** 标签中占满所在编辑组，随分栏宽高调整，提供编辑、行号、语法高亮、查找及缩略图。明确打开 Markdown 源码时才使用源码标签，保存后可通过 **打开 Markdown 渲染** 返回原生视图。Git 历史、双栏差异与搜索侧预览保持只读。
...
结果按文件分组，显示文件图标、名称、相对目录、匹配数、Git 状态与高亮片段；行列位置放在悬停提示和可访问标签中，不再单独占一列。文件分组和目录可折叠，可切换列表／树形显示，按路径／结果数排序，也可通过结果统计旁的 **在编辑器中打开** 展示完整结果。单击文件或片段仅更新下方预览；双击才按起止行列打开文件并选中精确匹配，Markdown 使用原生渲染。右键可在右侧打开、复制匹配行或路径。文件组悬停时可 **从结果中移除**，它只更新本轮结果，不删除磁盘文件。
...
Windows／Linux 的新窗口采用 Typora 原生 Unibody 样式，顶部使用 32 CSS px 的单行布局：24px 的随应用分发图标、中文主菜单、当前标题和原生窗口按钮共处一行。菜单按钮调用 Typora 原生命令入口，方向键可在菜单项之间移动；窗口较窄时隐藏中间标题，保留菜单和关闭按钮。顶部、侧栏与编辑区共用边界，侧栏高度扣除标题行，避免底部溢出。启用窗口样式使用 Typora 原生首选项；已有窗口不会被强制关闭，保存文档后正常重启即可更新其系统边框。
...
大纲移除最外层重复的 18px 左右留白，保留标题自身逐级缩进、折叠和点击跳转；悬停也不把行挤出侧栏。大纲上方不再提供原生过滤框及其关闭按钮，进入工作区时清理旧过滤状态，恢复被过滤的标题；搜索集中到搜索面板，切换侧栏不会把原生“查找”框带到那里。正文目录弹层的布局保持原样。
...
搜索标题只出现在侧栏顶部。下方预览的独立工具栏左侧为 **预览** 和收放按钮，右侧直接显示比例滑块与百分比；宽度充足时提供缩小、放大按钮。滑块、按钮和 Ctrl／Cmd 加滚轮同步更新同一个比例。

预览中，Markdown 按当前主题渲染标题、段落、列表和表格，代码围栏按语言着色，其他文本按语言映射显示只读 Monaco 源码；特殊文件名、复合后缀与解释器规则见 [全部文件与语言识别](#1.4.3_全部文件与语言识别)。命中映射统一处理 CRLF 与解析器展开的行首制表符，避免前面的代码块使后续位置偏移；改变侧栏状态也不会重置预览滚动。缩放以当前正文的实际字号为基准，同时调整 Markdown 和源码内容，不改变面板宽高或中央文档字号。预览自身可以上下、左右滚动，滚动与缩放不会移动中央文档。
