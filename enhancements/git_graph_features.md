...
界面共用 45 个官方 Codicons SVG，采用图标 CC BY 4.0／代码 MIT 许可，保留固定来源、摘要和随部署许可。按钮、状态栏及需要折叠的分组使用 SVG，箭头按开合状态旋转；不依赖本机 VS Code、字体或扩展。来源与原始画布见 [Codicons 说明](./vendor/codicons/README.md#第1章_Codicons图标来源与使用)。
...
侧栏历史采用 11px 轨道间距和 22px 行高，每行按本行仍存在的轨道计算图形宽度。提交前不另占箭头列，展开后直接显示带图标和状态的文件，不增加文件总数行；引用使用彩色圆角标签。大纲与搜索清除多余原生过滤框，大纲保留层级缩进并去除外围重复留白，见 [活动栏与侧栏布局](./README.md#1.4.5_活动栏与侧栏布局)。
...
Git 与其他主侧栏共用 170 CSS px 最小正文宽度；继续拖到请求宽度不足 85px 收起，活动栏仍可用于恢复，完整鼠标／键盘规则见 [活动栏与侧栏布局](./README.md#1.4.5_活动栏与侧栏布局)。该宽度和提交图内部上下分区、中央差异双栏宽度分别调整。
...
扩展回归覆盖真实鼠标的左右／上下分界线拖动、右键勾选持久化、列显隐菜单，以及终端的输入与生命周期。对应脚本见 [终端运行文件、安装与验证](./README.md#1.7_终端运行文件、安装与验证)。
...
侧栏回归还检查三视图显隐持久化、提交输入区折叠、提交下拉、历史文件树、行内按钮键盘操作、编辑框原生右键，以及 220px 窄侧栏中操作和状态列在悬停前后保持对齐。`test_git_sync.mjs` 覆盖真实本地远端分歧、拉取冲突、推送拒绝与上游变化；`test_git_discard_changes.mjs` 覆盖精确文件清单、暂存区字节保留、预览失效和回收失败。Windows Typora 实窗套件另检查系统回收站及状态栏同步确认，不使用用户远端或删除用户文件。

完整 `npm run check`、TypeScript 与最终安装一致性检查通过。Windows Typora 的最终 `workspace` 实窗通过 75 项，Git 实窗通过 66 项，重命名与 Markdown 原生精确定位各通过 16 项。预览隐藏 Electron 通过 31 项，搜索布局与 Explorer 各通过 20 项，Git Diff 全局状态通过 10 项；红绿概览、活动栏、大纲、终端主题及正则 Worker 另有专项覆盖。最终实窗画面已核对搜索标题、下方预览及直接缩放工具栏没有重叠。套件范围和结果见 [开发者构建与验证](./README.md#1.2_开发者构建)。
