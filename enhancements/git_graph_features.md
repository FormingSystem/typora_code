...
界面共用 42 个官方 Codicons SVG，采用图标 CC BY 4.0／代码 MIT 许可，保留固定来源、摘要和随部署许可。按钮、状态栏及折叠项使用 SVG，箭头按开合状态旋转；不依赖本机 VS Code、字体或扩展。来源与原始画布见 [Codicons 说明](./vendor/codicons/README.md#第1章_Codicons图标来源与使用)。
...
| 文件差异 | Monaco 左右／行内比较、对齐空行、行内差异、源码高亮、改动导航、可点击缩略图、两侧各 8px 滚动条与同步纵向滚动、30px 原生红绿差异概览、重命名前后路径 | 变更文件及其右键菜单 |
...
| 工作区集成 | 全文件与隐藏目录按需浏览、源码只读标签、按文件搜索及精确源码跳转、标签和分栏、文件菜单、状态栏、活动栏排序、仓库终端 | 工作区入口、设置 |
...
差异使用 MIT 许可的 **Monaco 0.56.0**，在中央编辑标签中显示完整双栏源码、行对齐、行内标色、语法高亮、中文查找和改动导航；两侧各保留 8px 滚动条，30px 原生概览分别显示删除与新增，点击后同步定位。功能说明见 [中央差异编辑器与文件时间线](./README.md#1.5.2_中央差异编辑器与文件时间线)。主侧栏复用社区核心 `SidebarPanel`，活动栏功能图标可排序；提交图内部不再复制侧栏和标签系统。终端继续使用现有 xterm.js 与 node-pty，颜色跟随 Typora 实际主题，管理员入口通过 UAC 打开独立 PowerShell，见 [集成终端说明](./README.md#1.6_集成终端、管理员入口与分界线)。

两个差异版本均为只读。**打开文件** 对 Markdown 使用 Typora 原生渲染与编辑，对其他文本使用占满编辑组的单栏只读 Monaco；当前没有普通源码保存、选中行暂存、三方合并编辑器或 VS Code 扩展宿主。二进制文件、目录及子模块不冒充文本差异，界面说明打开方式；符号链接比较链接值。文件编码由 `TextDecoder` 解码。

全文件浏览不会按已知语言隐藏文件，目录按需展开；特殊名称、最长复合后缀及解释器映射见 [全部文件与语言识别](./README.md#1.4.3_全部文件与语言识别)。工作区文本搜索与图内提交搜索是独立入口：前者提供文件分组、路径、行号、高亮和精确源码选区，以及大小写／全字／正则／包含排除／忽略／仅已打开与替换预览；写入前检查磁盘快照和未保存内容，范围与限制见 [工作区搜索与替换](./README.md#1.4.4_工作区搜索与替换)。这些能力不意味着已经移植完整 VS Code。
...
新增原生红绿概览、活动栏排序与减少动画、收起提交图防遮挡及终端主题分别有独立隐藏 Electron 回归；全文件、源码占满编辑组及搜索精确跳转已有 Windows Typora `1.14.9` 的 13 项 `browser` 实窗检查通过，同轮阅读、Git 与终端原生套件分别通过 18 项（15＋3）、66 项、13 项。搜索后台另通过匹配与替换保护、正则 Worker 取消和单文件超时回归。最新分层结果及本轮最终安装状态见 [开发者构建与验证](./README.md#1.2_开发者构建)，不把旧安装基线或独立夹具通过写成全部部署已经验收。
