...
VS Code 的 Diff Editor 在这里对应工作区中的 **只读双栏文本视图**，保留行号和增删标记；当前文件仍由 Typora 打开，二进制差异保留 Git 的提示。VS Code SCM 标题入口对应 Typora 的文件菜单、工具栏和状态栏；普通集成终端使用 xterm.js、node-pty 与独立后台进程，在仓库根目录打开工作区标签或分栏；管理员入口通过 UAC 打开独立 PowerShell。具体键位、安装与会话边界见 [集成终端说明](./README.md#1.6_集成终端、管理员入口与分界线)。文件编码使用 Chromium `TextDecoder` 支持的字符集，设置时检查有效性。字体、编辑器内部能力和 VS Code API 不属于这份功能对照的实现对象。
