...
工作台操作图标共用官方 Codicons SVG，采用图标 CC BY 4.0／代码 MIT 许可，保留固定来源、摘要和随部署许可。源代码管理更改行与侧栏历史展开文件按冻结基线使用官方通用文件图标；中央 Git Graph 扩展自身图标保持独立语义。按钮、状态栏及需要折叠的分组使用 SVG，箭头按开合状态旋转；不依赖本机 VS Code、字体或扩展。来源与原始画布见 [Codicons 说明](./vendor/codicons/README.md#第1章_Codicons图标来源与使用)。

[SCM 图标回归](./scripts/test_scm_file_icons.cjs) 验证列表／树视图的官方通用文件图标、22px 行高、实际点击比较、空分组箭头和按钮前景；这项冻结选择不宣称等同 VS Code 文件主题。

本轮已确认并修复显示缺陷：支线按自身颜色持续到父节点才汇入；独立stash节点只显示第一父边，stash私有index／untracked辅助提交不占历史行，普通引用仍可达的辅助对象保留。真实临时Git回归覆盖两个三父stash、正常引用保留和分页。SCM顶层标题只保留视图省略号，刷新和打开完整历史留在Graph标题区。模型与collector修复不修改Git对象；最终原生画面仍待集成验收。

## 1.1\_功能与入口

点击活动栏的 Git 图标，在原有主侧栏打开本地化的 **源代码管理**；与文件、搜索等宿主入口共用活动栏，可拖动排序。再次点击同一图标收起，点击其他功能切换侧栏；活动状态的背景与前景跟随实际面板，系统减少动态效果时关闭过渡。下半区 **提交图** 直接显示历史与选中提交的文件，上下分区支持拖动调整，收起后标题仍完整可见。原生文件树底部工具条不会覆盖提交图，正文状态栏保留。标题栏的打开按钮、状态栏 **Git Graph** 或提交图命令，在原有编辑标签栏打开完整历史；文件差异和时间线也使用该标签栏。布局与菜单见 [主侧栏与中文菜单](./README.md#1.5.1_主侧栏与中文菜单)。
...
文件行包含 16px 官方通用文件图标、名称和右侧 16px 状态固定列，行内操作仅在悬停或聚焦时覆盖名称尾部，不为操作长期预留空列。侧栏历史采用 11px 轨道间距和 22px 行高，每行按本行仍存在的轨道计算图形宽度。提交前不另占箭头列，展开后直接显示带图标和状态的文件，不增加文件总数行；引用使用彩色标签，其图标继承标签前景。大纲与搜索清除多余原生过滤框，大纲保留层级缩进并去除外围重复留白，见 [活动栏与侧栏布局](./README.md#1.4.5_活动栏与侧栏布局)。
...
以下属于承载 Git Graph 的冻结工作台入口，不是 Git Graph 扩展配置。2026-09-10 仅修已知问题；撤去的菜单重组、Command Center 和底部 Panel 不再列为现有能力。

| 功能／排版 | 当前入口与边界 | 证据状态 |
| --- | --- | --- |
| 七个主菜单 | 文件、编辑、段落、格式、视图、主题、帮助；各自对应 Typora 原生子菜单 | titlebar 目标已通过；真实子菜单主进程桥接正在集成，待统一构建与实窗验收 |
| 文件名查找 | 原生 `ClientCommand.quickOpen`，复用核心文件缓存；不保留自有 `>`、`@` 或行号解析面板 | 原生 browser fixture 使用真实宿主 DOM；本轮最终原生验证待运行 |
...
| 布局与图标 | 连续活动栏、独立原生大纲、普通编辑标签；窗控图标 18px 为本次用户覆盖，按钮仍 46×35px | 大纲、状态栏及空占位标签目标已过；完整集成待构建，详见[工作台问题矩阵](../docs/workbench_parity.md) |
...
设置作用于当前仓库。普通编辑显式保存后应用；导入验证成功后直接应用，但保留本机 Git 路径、终端与头像开关，导出屏蔽这些本机覆盖。保留未修改保存、存储失败回滚和仓库变化／写操作阻止保存的正确性检查。验证见 `test_git_graph_settings_view.cjs` 与原生 Git fixture。当前停止完整一比一功能移植，仅维护已有功能和已确认的缺陷修复。
