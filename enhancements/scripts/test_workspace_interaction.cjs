...
app.whenReady().then(async()=>{
  ...
  for(const theme of ['light','dark']){
    ...
    for(const zoom of [1,1.2,1.25]){
      ...
      for(const selector of ['.workspace-titlebar-menu>button','.workspace-titlebar-menu>button:nth-child(2)','#ribbon .group.bottom>button','#ribbon .group.bottom>div','[data-scm-title-action=refresh]','.typ-tab:not(.active)']){
