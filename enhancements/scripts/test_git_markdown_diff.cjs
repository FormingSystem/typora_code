...
app.whenReady().then(async()=>{
  ...
  await check('getComputedStyle(shadow.querySelector("[data-changed=true] [data-side=left] [data-source-line] > *")).backgroundColor!==getComputedStyle(shadow.querySelector("[data-changed=true] [data-side=right] [data-source-line] > *")).backgroundColor','左右实际绘制红绿差异色');
  await check('[...shadow.querySelectorAll(".markdown-diff-cell")].every(cell=>getComputedStyle(cell).backgroundColor==="rgba(0, 0, 0, 0)")','对齐空白不再涂整片差异色');
  await check('shadow.querySelector("strong [data-diff-inline=left]")?.textContent==="旧"&&shadow.querySelector("strong [data-diff-inline=right]")?.textContent==="新"','中文改词强调保留共同文字与粗体语义');
  await check('getComputedStyle(shadow.querySelector("strong [data-diff-inline=left]")).backgroundColor==="rgba(173, 7, 7, 0.15)"','行内差异复用上游透明色以叠加强调');
  await check('[...shadow.querySelectorAll("[data-side=right] [data-source-line] > p")].some(p=>getComputedStyle(p).boxShadow.includes("-4px"))','成对修改保留4px蓝色边栏');
  fs.writeFileSync(path.join(root,'inline_light.png'),(await win.webContents.capturePage()).toPNG());
