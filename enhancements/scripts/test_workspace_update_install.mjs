...
const root=fs.mkdtempSync(path.join(os.tmpdir(),'typora-update-install-')),checkout=path.join(root,'package','repository'),host=path.join(root,'host'),user_data=path.join(root,'custom profile 中文'),state_root=path.join(root,'updates'),checks=[];
fs.mkdirSync(checkout,{recursive:true});fs.mkdirSync(path.join(host,'resources'),{recursive:true});fs.mkdirSync(user_data,{recursive:true});fs.mkdirSync(state_root);
...
const env={...process.env,APPDATA:path.join(root,'appdata')};for(const key of Object.keys(env))if(key.toLowerCase()==='psmodulepath')delete env[key];
...
try{
 ...
 await service.run_worker(path.join(job,'request.json'),{request:async(_url,{file})=>fs.copyFileSync(archive,file)});
...
checks.push('真实ZIP解压、固定公告／资产校验、真实安装事务成功；非默认中文用户目录正确，设置和文档字节保留');
...
// 使用随安装交付的私有Node真正启动独立worker；同版本在联网之前拒绝。
...
checks.push('安装内私有Node可启动真实独立worker，同版本在下载前拒绝，进度文件可读取');
