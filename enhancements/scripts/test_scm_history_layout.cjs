...
app.whenReady().then(async()=>{
  ...
  for(const theme of ['light','dark']){
    ...
    const sample=await evaluate(`(()=>{const row=hash=>history_view.list.querySelector('[data-hash='+hash+']'),color=hash=>getComputedStyle(row(hash).querySelector('circle')).fill;return {future:color('future'),current:color('current'),older:color('older'),incoming:row('current').querySelector('.git-scm-history-topology path').getAttribute('stroke'),continuation:row('future').nextElementSibling.querySelector('line').getAttribute('stroke'),selected:history_view.list.querySelectorAll('[data-workspace-selected=true]').length}})()`);
    assert.equal(sample.future,'rgb(255, 176, 0)');assert.notEqual(sample.current,sample.future);assert.equal(sample.current,sample.older);assert.equal(sample.incoming,'#FFB000');assert.equal(sample.continuation,'#FFB000');assert.equal(sample.selected,1);
