...
app.whenReady().then(async()=>{
  ...
  assert.equal(await evaluate(`document.querySelector('[data-hash=branch_11]').nextElementSibling.querySelector('.git-scm-history-files').getBoundingClientRect().left-document.querySelector('[data-hash=branch_11]').getBoundingClientRect().left`),143,'all twelve branches keep their lanes until the common parent row');
