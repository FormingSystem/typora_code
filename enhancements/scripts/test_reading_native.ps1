...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot '../fixtures/reading_native_test.js') -Destination $probe_root
...
foreach ($phase in 1..2) {
    $script_uri = [Uri]::new((Join-Path $probe_root 'reading_native_test.js')).AbsoluteUri + '?phase=' + $phase
