...
app.whenReady().then(async()=>{
 ...
 const checks=await ev(`(async()=>{
  ...
  assert(!document.querySelector('[role=dialog]'),'设置是非模态编辑器');binding.show();assert(leaves.length===1,'重复打开复用标签');
  ...
  for(const width of [300,600]){document.querySelector('#group').style.width=width+'px';const content=document.querySelector('.workspace-settings');assert(content.scrollWidth<=width+1,'窄编辑组无横向溢出 '+width);}
