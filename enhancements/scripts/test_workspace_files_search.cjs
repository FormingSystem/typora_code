...
app.whenReady().then(async()=>{
  ...
  await evaluate(String.raw`(()=>{
    ...
    window.core={SidebarPanel:panel,WorkspaceView:view,app:{settings:{values:new Map(),get(key){return this.values.get(key)},set(key,value){this.values.set(key,value)},set_and_save(key,value){this.set(key,value)}},openFile:function(file){native_opens.push(file);return 'core-native-result'},workspace:workspace_api,viewManager:{registerView:(name,factory)=>factories.set(name,factory)},commands:{register:command=>commands.set(command.id,command),run:id=>commands.get(id)?.callback()}}};
  ...
  await wait('document.querySelector(".linux-note-workspace-search").dataset.state==="searching"&&document.querySelectorAll(".workspace-search-match").length>0');
  await evaluate('sidebar.activePanel.includes.value="./perf_*.txt";void 0');await set_input('搜索内容','progress_new');
  await wait('document.querySelector(".linux-note-workspace-search").dataset.state==="ready"&&document.querySelectorAll(".workspace-search-match").length===6');await delay(80);
  assert(await evaluate('[...document.querySelectorAll(".workspace-search-preview mark")].every(node=>node.textContent==="progress_new")'),'cancelling a large DOM build keeps only the current query');
  assert.equal(fs.readFileSync(path.join(workspace,'dense_perf.txt'),'utf8'),'dense_marker\n'.repeat(5000));
  await evaluate(`(()=>{window.release_search_finish=null;let held=false;files.fs.promises.readFile=async(...args)=>{if(!held&&String(args[0]).endsWith('perf_0011.txt')){held=true;await new Promise(resolve=>release_search_finish=resolve);}return original_search_read(...args)};})()`);
  await set_input('搜索内容','progress_old');await wait('!!release_search_finish&&document.querySelectorAll(".workspace-search-match").length===6');
  await evaluate('window.retained_search_row=document.querySelector(".workspace-search-match");retained_search_row.focus();release_search_finish();void 0');
