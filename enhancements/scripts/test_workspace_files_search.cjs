...
app.whenReady().then(async()=>{
  ...
  const calls_before_open=await evaluate('open_calls.length');await click(match_selector);await wait('open_calls.length>'+calls_before_open);assert.equal(await evaluate('open_calls.at(-1)[1].preview'),true,'single click uses editor preview by default');assert.equal(await evaluate('document.querySelector(".workspace-search-preview-section").hidden'),true,'reading extension does not occupy the default search layout');
  ...
  await evaluate(`document.querySelector(${JSON.stringify(match_selector)}).dispatchEvent(new MouseEvent('dblclick',{bubbles:true}))`);await wait('open_calls.length>'+calls_before_preview);assert.deepEqual(await evaluate('open_calls.at(-1)[1]'),{line:3,column:1,end_line:3,end_column:7,source:false,expected_text:'NEEDLE',preview:false,preserve_focus:false});
