...
app.whenReady().then(async()=>{
  ...
  await evaluate(`core.app.openFile(${JSON.stringify(path.join('src','space name [draft] #100%.test.ts'))})`);await delay(80);
  assert.equal(await evaluate('core.app.workspace.activeLeaf.view.file_path'),complex_source,'relative application routes resolve from the workspace context root');
  ...
  assert.equal(layout.native,'none');assert.equal(layout.offset,0);assert.equal(layout.heading_buttons,4);assert(layout.replace_hidden);assert(layout.query_height<=28);assert.equal(layout.outline,'0px');assert.equal(layout.shadow,'none');assert(layout.toggle_boxes);
  ...
  assert.equal(await evaluate('document.querySelectorAll(".workspace-search-file>summary>svg[data-git-icon=file]").length'),3);
  ...
  assert.deepEqual(dispose_state,{core:true,library:true,host:true,marker:true,source_marker:true},'dispose restores the original routes while retaining the single reusable file host');
  ...
  assert.deepEqual(rebound_state,{same:true,idempotent:true,core:true,library:true,styles:true},'a released host reinstalls one routing layer without duplicating UI bindings');
