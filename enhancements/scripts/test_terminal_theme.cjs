...
app.whenReady().then(async () => {
  ...
  await evaluate(`(() => {
    ...
    const factories=window.terminal_factories=new Map(); const leaves=window.terminal_leaves=[]; window.terminal_registrations=0; const registered=()=>{terminal_registrations++;return()=>terminal_registrations--;};
    const core={WorkspaceView:class{constructor(leaf){this.leaf=leaf;}},app:{viewManager:{registerView:(type,factory)=>{factories.set(type,factory);return()=>factories.delete(type);}},commands:{run(){},register:registered},workspace:{activeLeaf:null,eachLeaves:callback=>leaves.forEach(callback),on:registered,ribbon:{addButton:registered}}}};
    const parent={removeTab(path){const index=leaves.findIndex(leaf=>leaf.state.path===path);if(index>=0){const leaf=leaves.splice(index,1)[0];leaf.view.onClose();if(core.app.workspace.activeLeaf===leaf)core.app.workspace.activeLeaf={parent};}},appendChild(leaf){leaves.push(leaf);document.querySelector('#host').append(leaf.view.containerEl);leaf.view.start=()=>{leaf.view.status.textContent='现有终端输出';};leaf.view.onOpen();}};
    ...
    const host={core,fs:require('node:fs'),path_api:require('node:path'),process_api:process,context_path:()=>${JSON.stringify(root)},runner:()=>({run:()=>new Promise(resolve=>window.resolve_terminal_root=resolve)})};
    localStorage.setItem('linux-note-terminal:v1:',JSON.stringify({profile:'cmd',location:'active'}));window.terminal_host=host;window.terminal_binding=terminal_qa.bind_terminal_workspace(host);terminal_binding.open(${JSON.stringify(root)},'cmd','active');
  ...
  await evaluate('window.preserved_term=view.term;window.preserved_leaf=view.leaf;window.pty_kills=0;window.preserved_pty=view.pty={pid:42,write(){},resize(){},acknowledge(){},kill(){pty_kills++;}};view.term.select(0,0,16);window.preserved_selection=view.term.getSelection();window.root_bottom_before=document.querySelector("#host").style.bottom;terminal_binding.move_to_panel();void 0');await delay(100);
  assert(await evaluate('view.term===preserved_term&&view.pty===preserved_pty&&view.leaf===preserved_leaf&&view.term.getSelection()===preserved_selection&&pty_kills===0&&document.querySelector(".typora-terminal-panel-body").contains(view.containerEl)&&terminal_leaves.length===0'));
  const panel_geometry=await evaluate('(()=>{const panel=document.querySelector(".typora-terminal-panel").getBoundingClientRect(),root=document.querySelector("#host").getBoundingClientRect();return{panel_top:panel.top,panel_bottom:panel.bottom,root_bottom:root.bottom,footer:innerHeight-22}})()');
  assert(Math.abs(panel_geometry.root_bottom-panel_geometry.panel_top)<=1&&Math.abs(panel_geometry.panel_bottom-panel_geometry.footer)<=1);
  await evaluate('document.querySelector(".typora-terminal-panel-heading button[aria-label=隐藏面板]").click()');
  assert(await evaluate('document.querySelector("#host").style.bottom===root_bottom_before&&view.pty===preserved_pty&&pty_kills===0'));
  await evaluate('terminal_binding.move_to_panel();terminal_binding.move_to_editor();void 0');await delay(100);
  assert(await evaluate('view.term===preserved_term&&view.pty===preserved_pty&&view.term.getSelection()===preserved_selection&&terminal_leaves.length===1&&document.querySelector("#host").style.bottom===root_bottom_before&&pty_kills===0'));
  checks.push('moving the same terminal between editor and bottom Panel preserves leaf xterm PTY buffer and selection without restart');
  checks.push('bottom Panel aligns above the 22px footer, reserves editor height and restores original root bottom when hidden');
  await evaluate('terminal_binding.move_to_panel();void 0');await delay(50);
  fs.writeFileSync(path.join(evidence,'terminal_bottom_panel.png'),(await test_window.webContents.capturePage()).toPNG());
  ...
  await evaluate('terminal_binding.open(terminal_host.context_path(),"cmd","active");window.dispatchEvent(new CustomEvent("linux-note-open-terminal",{detail:{path:terminal_host.context_path()}}))'); await delay(50);
  ...
  assert(await evaluate('terminal_factories.size===1&&terminal_registrations===8&&document.querySelectorAll("[data-workspace-terminal-style]").length===1'));
  ...
  assert(await evaluate('!document.querySelector(".typora-terminal-panel").hidden&&terminal_leaves.length===0&&Boolean(view.term)'));
  const panel_height=await evaluate('document.querySelector(".typora-terminal-panel").getBoundingClientRect().height');
  await evaluate('document.querySelector(".typora-terminal-panel-sash").dispatchEvent(new KeyboardEvent("keydown",{key:"ArrowUp",bubbles:true}))');
  assert.equal(await evaluate('document.querySelector(".typora-terminal-panel").getBoundingClientRect().height'),panel_height+10);
  await evaluate('terminal_binding.dispose()');
  assert(await evaluate('!document.querySelector(".typora-terminal-panel")&&document.querySelector("#host").style.bottom===root_bottom_before'));
  checks.push('new default terminal opens in the bottom Panel and keyboard sash resizing restores host geometry on unload');
