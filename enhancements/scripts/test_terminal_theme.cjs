...
app.whenReady().then(async () => {
  ...
  assert(await evaluate('terminal_factories.size===1&&terminal_registrations===5&&document.querySelectorAll("[data-workspace-terminal-style]").length===1'));
  ...
  assert(await evaluate('split_call.id==="core.workspace:split-down"&&split_call.args[0].startsWith("typ://linux_note.terminal/")&&!document.querySelector(".typora-terminal-panel")'));
  await evaluate('terminal_binding.open(terminal_host.context_path(),"cmd","active");void 0');await delay(50);
  assert(await evaluate('terminal_leaves.length===1&&Boolean(view.term)&&document.querySelector("#host").contains(view.containerEl)'));
  ...
  checks.push('default terminal uses the original lower editor group; explicit active location opens a normal editor tab');
