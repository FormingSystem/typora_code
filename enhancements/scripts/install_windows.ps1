...
Write-Host "已安装全文件目录（含隐藏文件）、复合后缀识别、源码标签与文件搜索预览。"
Write-Host "源代码管理使用官方 Codicons；包含紧凑提交图、Ctrl+Enter 提交、双栏差异和红绿概览；全文缩略图只在普通文档显示。"
Write-Host "搜索支持输入关键词和选中文字后 Ctrl+左键：单击仅预览，双击打开并定位；预览可收放，下方工具栏直接提供百分比滑块，并与 Ctrl+滚轮同步。"
Write-Host "普通源码支持 Ctrl+S 保存、Ctrl+F 查找；全局底栏随当前编辑区更新行列、语言模式、编码和换行格式。"
Write-Host "Windows／Linux 新窗口采用单行标题与菜单，保留 Typora 原生窗口按钮；请保存文档后重启使已有窗口生效。"
Write-Host "活动栏支持排序，终端跟随主题。Git 从 PATH 发现；无需独立安装 Node.js。"
