...
app.whenReady().then(async()=>{
  ...
  const indentation=await evaluate(`(()=>{const parent=document.querySelector(${JSON.stringify(row('.hidden'))}),child=document.querySelector(${JSON.stringify(row(path.join('.hidden','.secret.c')))});return {step:parseInt(child.style.paddingLeft)-parseInt(parent.style.paddingLeft),slot:getComputedStyle(child.querySelector('.workspace-explorer-chevron')).display,icon:child.querySelector('.workspace-file-theme-icon').getBoundingClientRect().width};})()`);
  assert.deepEqual(indentation,{step:8,slot:'none',icon:16},'Seti leaf removes empty twistie and each tree level uses the upstream 8px indent');
