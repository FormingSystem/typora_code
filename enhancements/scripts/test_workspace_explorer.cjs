...
app.whenReady().then(async()=>{
  ...
  await evaluate(String.raw`(()=>{
    ...
    window.instance=explorer_qa.bind_workspace_explorer(core,{compact_folders:false,context_root:()=>root_path,active_file:()=>active_path,open_file:(...args)=>opened.push(args),open_folder:()=>{},copy:text=>copied.push(text),create:(root,parent,name,directory)=>explorer_qa.create_workspace_entry({fs:native_fs,path_api:require('path')},root,parent,name,directory),file_clipboard,trash:(root,paths)=>explorer_qa.trash_workspace_entries({fs:native_fs,path_api:require('path')},root,paths,async file=>{const recycle=require('path').join(require('path').dirname(root),'recycle');await native_fs.promises.mkdir(recycle,{recursive:true});await native_fs.promises.rename(file,require('path').join(recycle,require('path').basename(file)))}),rename:async(root,source,name)=>{const plan=await explorer_qa.prepare_workspace_rename({fs:native_fs,path_api:require('path')},root,source,name);await plan.apply();return plan.new_path;}});
