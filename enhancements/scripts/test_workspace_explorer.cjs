...
app.whenReady().then(async()=>{
  ...
  if(process.platform==='win32')assert.deepEqual(await evaluate('double_click_queries.map(item=>[item.file,item.args,item.options.windowsHide,item.options.timeout])'),[['reg.exe',['query','HKCU\\Control Panel\\Mouse','/v','DoubleClickSpeed'],true,500]]);
  ...
  for(const type of ['mouseMove','mouseDown','mouseUp']){test_window.webContents.sendInputEvent({type,...double_point,button:'left',clickCount:1});await delay(25)}
  await delay(600);assert.equal(await evaluate('opened.length'),double_open_count,'a first click stays pending for the configured system double-click window');
  for(const type of ['mouseMove','mouseDown','mouseUp']){test_window.webContents.sendInputEvent({type,...double_point,button:'left',clickCount:2});await delay(25)}
  await wait('!!document.querySelector(".workspace-explorer-rename")');assert.equal(await evaluate('opened.length'),double_open_count,'a recognized slow double-click renames without opening the file');await evaluate('document.querySelector(".workspace-explorer-rename").value="second_name.py"');await key('Enter');await wait(`!!document.querySelector(${JSON.stringify(row('second_name.py'))})`);assert(fs.existsSync(path.join(workspace,'second_name.py')));
  const single_open_count=await evaluate('opened.length');await click(row('second_name.py'));assert.equal(await evaluate('opened.length'),single_open_count);await wait('opened.length>'+single_open_count);assert.equal((await evaluate('opened')).at(-1)[0],path.join(workspace,'second_name.py'));
