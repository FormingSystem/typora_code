...
app.whenReady().then(async()=>{
  ...
  await evaluate(String.raw`(()=>{
    ...
    window.instance=explorer_qa.bind_workspace_explorer(core,{compact_folders:false,context_root:()=>root_path,active_file:()=>active_path,open_file:(...args)=>opened.push(args),open_folder:()=>{},copy:text=>copied.push(text),create:(root,parent,name,directory)=>explorer_qa.create_workspace_entry({fs:native_fs,path_api:require('path')},root,parent,name,directory),transfer:(root,paths,target,move)=>explorer_qa.transfer_workspace_entries({fs:native_fs,path_api:require('path')},root,paths,target,move?async(_root,source,target)=>{await native_fs.promises.rename(source,target);return target}:undefined),trash:(root,paths)=>explorer_qa.trash_workspace_entries({fs:native_fs,path_api:require('path')},root,paths,async file=>{const recycle=require('path').join(require('path').dirname(root),'recycle');await native_fs.promises.mkdir(recycle,{recursive:true});await native_fs.promises.rename(file,require('path').join(recycle,require('path').basename(file)))}),rename:async(root,source,name)=>{const plan=await explorer_qa.prepare_workspace_rename({fs:native_fs,path_api:require('path')},root,source,name);await plan.apply();return plan.new_path;}});
  ...
  await click(row('renamed_file.py'));assert.equal(await evaluate('opened.length'),double_open_count+1,'single click opens immediately');assert.equal((await evaluate('opened')).at(-1)[1].preview,false,'single click keeps the file open under the frozen layout');
  await evaluate(`document.querySelector(${JSON.stringify(row('renamed_file.py'))}).dispatchEvent(new MouseEvent('dblclick',{bubbles:true,detail:2}))`);
  await wait('opened.length>'+String(double_open_count+1));assert.equal(await evaluate('document.querySelector(".workspace-explorer-rename")'),null,'double click opens without renaming');assert.equal((await evaluate('opened')).at(-1)[1].preview,false,'double click requests keep-open');await evaluate(`document.querySelector('.workspace-explorer-tree').focus()`);await key('Enter');assert.equal((await evaluate('opened')).at(-1)[1].preview,false,'Enter requests keep-open');
  ...
  const root_menu = async title => { await evaluate(`document.querySelector('.workspace-explorer-tree').dispatchEvent(new MouseEvent('contextmenu',{bubbles:true}));[...document.querySelectorAll('[role=menu] button')].find(button=>button.textContent===${JSON.stringify(title)}).click()`); await delay(100); };
  ...
  await evaluate(`document.querySelector('.workspace-explorer-tree').focus()`);await key('Delete');await wait('!!document.querySelector("[role=dialog]")');await evaluate('[...document.querySelectorAll("[role=dialog] button")].find(button=>button.textContent==="移到回收站").click()');await wait(`!document.querySelector(${JSON.stringify(row(path.join('created_folder','renamed_file.py')))})`);assert(fs.existsSync(path.join(root,'recycle','renamed_file.py')));
