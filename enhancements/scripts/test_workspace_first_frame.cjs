...
app.whenReady().then(async()=>{
 ...
 assert(!production.style_inputs.some(file=>file.endsWith('_shadow.css')),'Shadow CSS must never enter the global asset');
 assert(!fs.readFileSync(production.css_path,'utf8').includes(':host {display:block;min-width:0;font:inherit'),'Shadow host rules cannot leak into document styles');
 ...
 const compiled=await build({stdin:{contents:'export {acquire_workspace_style} from "./src/workspace_styles";export {create_workspace_hover_markdown} from "./src/workspace_hover_markdown";',resolveDir:path.join(__dirname,'..')},plugins:[static_workspace_css_plugin()],bundle:true,write:false,format:'iife',globalName:'qa'});
 ...
 assert(await run('(()=>{const host=qa.create_workspace_hover_markdown("- item",()=>{});document.body.append(host);const padding=getComputedStyle(host.shadowRoot.querySelector("ul")).paddingLeft;host.remove();return padding==="20px"&&document.querySelectorAll("style").length===before_style_count})()'),'production plugin preserves scoped Shadow styles without global injection');
