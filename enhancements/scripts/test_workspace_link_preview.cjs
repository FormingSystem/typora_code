...
app.whenReady().then(async()=>{
  ...
  await evaluate(`window.dock_binding=qa.bind_workspace_link_dock(core,files);window.dock=document.querySelector('.workspace-link-dock');window.pointer=node=>node.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,composed:true,button:0}));window.pick=()=>{pointer(document.querySelector('#write a'));select_link();document.dispatchEvent(new Event('selectionchange'));};pick();`);
  const settle=()=>new Promise(r=>setTimeout(r,180));await settle();
  check('浮动预览默认未固定且图标可访问',await evaluate(`!dock.hidden&&dock.querySelector('[aria-label="固定链接预览"]').getAttribute('aria-pressed')==='false'`));
  await evaluate(`pointer(document.body);document.dispatchEvent(new Event('pointerup'));document.dispatchEvent(new Event('selectionchange'));`);await settle();
  check('空白处收起且旧选区不会重新弹出',await evaluate(`dock.hidden&&!dock.querySelector('.workspace-lookup-markdown')`));
  await evaluate('pick()');await settle();check('同一链接明确重新选择可打开',await evaluate('!dock.hidden'));
  await evaluate(`pointer(dock.querySelector('.workspace-lookup-markdown').shadowRoot.querySelector('p'));document.dispatchEvent(new Event('pointerup'));`);await settle();check('Shadow内部选中不关闭',await evaluate('!dock.hidden'));
  await evaluate(`pointer(dock.querySelector('.workspace-preview-sash'));document.body.dispatchEvent(new Event('pointerup',{bubbles:true}));`);check('从预览边界开始的拖动保持显示',await evaluate('!dock.hidden'));
  await evaluate(`dock.querySelector('[aria-label="固定链接预览"]').click();pointer(document.body);`);await settle();check('固定后外部操作保留预览且状态明确',await evaluate(`!dock.hidden&&dock.querySelector('[aria-label="取消固定链接预览"]').getAttribute('aria-pressed')==='true'`));
  await evaluate(`dock.querySelector('[aria-label="取消固定链接预览"]').click();`);check('取消固定本身不关闭',await evaluate('!dock.hidden'));
  await evaluate(`window.tool=document.createElement('button');tool.textContent='外部工具';document.body.append(tool);tool.focus();tool.dispatchEvent(new FocusEvent('focusin',{bubbles:true,composed:true}));`);await settle();check('键盘焦点移到其他功能收起',await evaluate('dock.hidden'));
  await evaluate('pick()');await settle();
  ...
  await evaluate(`pointer(tool);document.querySelector('#write a').setAttribute('href','target.md#target-heading');`);await settle();
  for(let i=0;i<20;i++){await evaluate('pick()');await settle();check('外部关闭往返 '+i,await evaluate(`!dock.hidden`));await evaluate('pointer(tool)');await settle();if(!await evaluate('dock.hidden'))throw Error('关闭失败 '+i);}
  await evaluate(`pick();tool.style.cssText='position:fixed;left:700px;top:10px;width:100px;height:30px';window.trusted_outside=false;tool.addEventListener('pointerdown',e=>trusted_outside=e.isTrusted);`);await settle();
  win.webContents.sendInputEvent({type:'mouseDown',x:750,y:25,button:'left',clickCount:1});win.webContents.sendInputEvent({type:'mouseUp',x:750,y:25,button:'left',clickCount:1});await settle();
  check('真实Chromium外部鼠标操作收起',await evaluate('trusted_outside&&dock.hidden'));
  await evaluate('pick()');await settle();await evaluate(`window.other_preview=document.createElement('section');other_preview.className='workspace-lookup-preview';document.body.append(other_preview);pointer(other_preview);`);await settle();check('其他搜索预览是外部操作且其内容不受影响',await evaluate(`dock.hidden&&other_preview.isConnected`));await evaluate('other_preview.remove()');
  await evaluate(`window.saved_read_text=files.read_text;window.release_dock_read=undefined;files.read_text=()=>new Promise(resolve=>{release_dock_read=resolve});pick();`);await settle();
  check('异步预览开始读取',await evaluate(`!!release_dock_read&&!dock.hidden&&dock.querySelector('.workspace-link-preview').dataset.state==='loading'`));
  await evaluate(`pointer(tool);release_dock_read('# Late preview');files.read_text=saved_read_text;`);await settle();check('关闭后迟到文件读取不能重开或重新挂载正文',await evaluate(`dock.hidden&&!dock.querySelector('.workspace-lookup-markdown')`));
  await evaluate(`pick();pointer(tool);`);await settle();check('选区延迟尚未完成时外部操作取消开启',await evaluate('dock.hidden'));
  await evaluate(`pick()`);await settle();await evaluate(`dock.querySelector('[aria-label="固定链接预览"]').click();window.dispatchEvent(new Event('linux-note-workspace-context-changed'));`);check('切库清除固定及内容',await evaluate(`dock.hidden&&dock.querySelector('[aria-label="固定链接预览"]')!==null`));
  await evaluate('dock_binding.dispose();tool.remove();pick()');await settle();check('销毁后外部选择不重建dock',await evaluate(`!document.querySelector('.workspace-link-dock')`));
