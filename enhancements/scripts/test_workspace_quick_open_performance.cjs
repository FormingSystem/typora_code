...
app.whenReady().then(async()=>{
  ...
  for(const source_ref of baseline?[baseline,undefined]:[undefined]){
    ...
    await wait(`document.querySelectorAll('.workspace-quick-open-result').length===100&&!document.querySelector('.workspace-quick-open-status').textContent.includes('正在查找')`);
    ...
    await wait(`document.querySelectorAll('.workspace-quick-open-result').length===100&&[...document.querySelectorAll('.workspace-quick-open-name')].every(node=>node.textContent.startsWith('file_49'))`);
    if(!source_ref){
      ...
      await wait(`[...document.querySelectorAll('.workspace-quick-open-name')].length===100&&[...document.querySelectorAll('.workspace-quick-open-name')].every(node=>node.textContent.startsWith('file_47'))`);
      ...
      await evaluate('picker.open();void 0');await wait(`document.querySelectorAll('.workspace-quick-open-result').length===100&&!document.querySelector('.workspace-quick-open-status').textContent.includes('正在查找')`);
  ...
  console.log(JSON.stringify({status:'PASS',checks:['50000-name catalogue uses bounded top100 selection','query computation yields to Chromium input','latest query wins during sliced ranking','close cancels rendering and restores focus'],metrics,evidence:root},null,2));
