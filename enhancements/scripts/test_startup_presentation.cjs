...
app.whenReady().then(async()=>{
 ...
 for(const theme of ['light','dark'])for(const outcome of ['ready','failure','timeout']){
  ...
  fs.writeFileSync(html,`<!doctype html><html><head><meta charset="UTF-8"><style>${css}</style><script>window.real_timeout=setTimeout;window.setTimeout=(fn,ms)=>{if(ms===15000){window.deadline=fn;return 0;}return real_timeout(fn,ms)};</script><script>${head}</script></head><body class="typora-node unibody-window" style="--text-color:${theme==='dark'?'#ddd':'#333'};background:${theme==='dark'?'#222':'#fff'}"><div id="top-titlebar"><button id="w-close">Close</button><span id="title-text">Native</span><div class="workspace-titlebar-menu">Final menu</div></div><div class="typ-workspace-root">Final workbench</div><content>Saved document</content><div id="typora-sidebar">Native sidebar</div><footer class="ty-footer">Native footer</footer></body></html>`,'utf8');
  ...
  assert.equal(await run('getComputedStyle(document.querySelector(".typ-workspace-root")).visibility'),'hidden','核心根不能在增强就绪前暴露半成品');
  assert.equal(await run('getComputedStyle(document.querySelector(".workspace-titlebar-menu")).visibility'),'hidden');
  assert(!(await run('getComputedStyle(document.body,"::after").content')).includes('正在加载工作台'),'没有加载提示覆盖层');
  await run('window.original_root=document.querySelector(".typ-workspace-root");window.original_content=document.querySelector("content");void 0');
  ...
  assert.equal(await run('getComputedStyle(original_root).visibility'),'visible');
  assert(await run('original_root===document.querySelector(".typ-workspace-root")&&original_content===document.querySelector("content")'),'完成与失败不替换原节点');
