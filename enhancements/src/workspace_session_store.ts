...
export function create_workspace_session_store(fs:any,path_api:any,crypto:any,directory:string){
  ...
  const validate=(value:any,root:string):workspace_session=>{
    if(value?.schema!==1||value.root!==root_key(root)||!Array.isArray(value.files)||!Number.isInteger(value.active)||value.active< -1||value.active>=value.files.length)throw new Error("工作区会话记录无效。");
    if(value.files.some((file:any)=>!file||typeof file.path!=="string"||file.path.includes("\0")||!path_api.isAbsolute(file.path)||typeof file.source!=="boolean"||typeof file.pinned!=="boolean"))throw new Error("工作区会话文件身份无效。");
    return value;
  };
  return {root_key,
