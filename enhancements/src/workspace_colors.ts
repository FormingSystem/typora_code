...
export function bind_workspace_colors(){
  ...
  return{dispose(){if(disposed)return;disposed=true;release();style.remove();if(previous===null)root.removeAttribute('data-workspace-colors');else root.setAttribute('data-workspace-colors',previous);}};
}
