...
/** 恢复旧底部齿轮；仅打开 Typora 原生偏好，不注册插件设置或快捷键。 */
export function bind_workspace_preferences(core:graph_core) {
  ...
  const binding={dispose(){if(disposed)return;disposed=true;observer.disconnect();button.onclick=null;button.remove();style.remove();bindings.delete(core);}};
