...
export function bind_workspace_native_toolbar(files:workspace_file_host,runtime:{File?:any}){
  ...
  let toolbar:HTMLElement|undefined,toolbar_height=0,frame=0;
  ...
  const layout=()=>{
    ...
    if(node instanceof HTMLElement&&node!==toolbar){
      ...
      node.dataset.workspaceNativeToolbar="ready";resize.observe(node);mutation.observe(node,{attributes:true,attributeFilter:["style","class"]});
    }
    if(!toolbar)return;
  ...
  const root=document.querySelector(".typ-workspace-root");if(root)resize.observe(root);
