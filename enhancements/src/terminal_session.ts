...
export class terminal_session {
    ...
    try{
      ...
      const pty=await start_terminal_pty({signal:startup.signal,child_process:runtime.reqnode("child_process"),process_api:this.host.process_api,
        ...
        {executable:launch.executable,args:launch.args,options:{name:"xterm-256color",cols:start_cols,rows:start_rows,cwd:launch.cwd,env:launch.env,useConpty:true,useConptyDll:true}}, {
          ...
          exit:code=>{if(generation!==this.generation||this.disposed)return;this.ssh_auth?.dispose();this.ssh_auth=undefined;this.pty=undefined;this.pid=0;this.state="exited";this.launch_pending=false;this.status=`Shell 已退出（${code}）`;this.changed();},
  ...
  stop(){this.generation++;this.ssh_auth?.dispose();this.ssh_auth=undefined;this.startup?.abort();this.startup=undefined;const pty=this.pty;this.pty=undefined;pty?.kill();this.pid=0;this.state="exited";this.launch_pending=false;this.status="Shell 已终止";}
