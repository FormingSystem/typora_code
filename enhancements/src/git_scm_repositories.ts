...
export class git_scm_repositories {
  ...
  constructor(public owner:git_source_control){this.container.setAttribute("aria-label",text("scm.repositories"));}
  refresh():void{
  ...
  dispose():void{this.epoch++;this.reader?.dispose?.();this.reader?.cancel();this.update_rows=[];}
}
