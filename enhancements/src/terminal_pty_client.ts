...
export function start_terminal_pty(runtime: { child_process: any; process_api: any; broker: string; executable: string }, request: object,
  ...
  return new Promise((resolve, reject) => {
    ...
    const proxy: terminal_pty = { pid: 0,
      ...
      kill() { if (stopped) return; clearTimeout(timeout); send({ type: 'close' }); stopped = true; setTimeout(() => { if (child.connected) child.disconnect(); }, 1500); },
    };
    child.stderr?.on('data', (data: any) => { stderr = (stderr + String(data)).slice(-4000); });
    child.on('message', (message: any) => {
      if (stopped) return;
      if (message.type === 'ready') { ready = true; clearTimeout(timeout); proxy.pid = message.pid; resolve(proxy); }
    ...
    child.on('error', (error: Error) => { clearTimeout(timeout); if (!ready) reject(error); else callbacks.error(error.message); stopped = true; });
    child.on('exit', (code: number) => { clearTimeout(timeout); if (!ready) reject(new Error('终端宿主启动失败：' + stderr)); else if (!stopped) callbacks.exit(code); stopped = true; });
