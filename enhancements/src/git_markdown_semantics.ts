import {highlight_markdown_inline} from './git_markdown_inline';
...
export function refine_markdown_change(row:HTMLElement):HTMLElement[]{
  ...
  if(!a||!b||a.tagName!==b.tagName||left.querySelectorAll(':scope > [data-source-line]').length!==1||right.querySelectorAll(':scope > [data-source-line]').length!==1){
    const x=left.querySelector<HTMLElement>(':scope > [data-source-line]'),y=right.querySelector<HTMLElement>(':scope > [data-source-line]');
  ...
  const replacement=(xs:HTMLElement[],ys:HTMLElement[])=>{for(let k=0;k<Math.max(xs.length,ys.length);k++){
    const x=xs[k],y=ys[k];if(table&&x&&y&&x.children.length===y.children.length){for(let c=0;c<x.children.length;c++)if(x.children[c].outerHTML!==y.children[c].outerHTML){paint(x.children[c] as HTMLElement,'left');paint(y.children[c] as HTMLElement,'right');highlight_markdown_inline(x.children[c] as HTMLElement,y.children[c] as HTMLElement);}}
    else{if(x)paint(x,'left');if(y)paint(y,'right');if(x&&y)highlight_markdown_inline(x,y);}
  }};
  let ai=0,bi=0;
