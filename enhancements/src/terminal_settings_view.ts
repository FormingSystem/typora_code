...
export function show_terminal_settings(store:terminal_settings_store,dialog:ReturnType<typeof workspace_dialog>){
  ...
  const profile=el("select");profile.append(option("","默认"));for(const item of store.profiles())profile.append(option(item.id,item.title));profile.value=current.profile;row("profile","默认配置",profile);
  ...
  for(const[key,label,help]of [["env","环境变量（JSON）","字符串设置变量，null 删除变量；更改在新建或重启后生效。"],["profiles","自定义 Shell 配置（JSON）","每项包含 id、title、executable、args，可附 cwd、env、icon、color。相同 id 覆盖内置配置。"]]as const){const control=el("textarea");control.rows=key==="profiles"?10:4;control.value=JSON.stringify(current[key],null,2);control.spellcheck=false;row(key,label,control,help);}
