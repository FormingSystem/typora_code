...
export const graph_defaults = {
  ...
  details_location: "right", auto_center: true, file_view: "tree", compact_folders: true,
...
export const settings_labels: Record<keyof graph_settings, string> = {
  ...
  details_location: "详情位置（right / bottom / inline）", auto_center: "选中提交自动居中", file_view: "文件视图（tree / list）", compact_folders: "合并单子目录", label_alignment: "引用位置（inline / split / graph）", combine_refs: "合并同名本地和远端引用", uncommitted_style: "未提交节点（row / connected）", inline_markdown: "提交说明行内 Markdown",
  ...
  encoding: "历史文件编码", git_path: "Git 可执行文件", terminal_shell: "外部终端程序（空为系统默认）", new_tab_group: "文件与差异打开位置（active / right / down）", open_active_repo: "从活动文档查找仓库", search_depth: "子仓库发现深度", repository_order: "仓库排序（name / path / recent）", show_status_button: "显示状态栏入口", file_menu_entry: "显示文件菜单入口", icon_color: "入口图标颜色（auto 或 CSS 颜色）",
};
