...
function validate_manifest(value,{host_version,platform=process.platform,core_version='2.10.15'}={}){
 ...
 for(const key of ['name','description','author'])if(typeof value[key]!=='string'||key==='name'&&!value[key].trim())throw Error('插件清单缺少有效的'+key+'。');
...
async function verify_directory(directory){
 ...
 const walk=async dir=>{for(const name of (await fs.promises.readdir(dir)).sort()){
  ...
  if(stat.isDirectory())await walk(file);else {hash.update(path.relative(directory,file).split(path.sep).join('/'));hash.update('\0');for await(const chunk of fs.createReadStream(file)){hash.update(chunk);}}
 }};
...
function create_community_service({root,host_version,load_plugin,unload_plugin,request,extract,acquire_lock,platform=process.platform}){
 ...
 const json_request=async url=>JSON.parse((await request(url,{})).toString('utf8'));
 async function catalog(){const head=await json_request('https://api.github.com/repos/typora-community-plugin/typora-plugin-releases/commits/main');if(!/^[a-f0-9]{40}$/.test(head.sha))throw Error('社区目录版本身份无效。');const url='https://raw.githubusercontent.com/typora-community-plugin/typora-plugin-releases/'+head.sha+'/';let rows;try{rows=await json_request(url+'community-plugins.zh-cn.json');}catch{rows=await json_request(url+'community-plugins.json');}if(!Array.isArray(rows))throw Error('社区目录格式无效。');return rows.filter(row=>{try{plugin_id(row.id);return /^[\w.-]+\/[\w.-]+$/.test(row.repo)&&typeof row.name==='string'&&typeof row.description==='string';}catch{return false;}});}
 async function latest(info){plugin_id(info.id);if(!/^[\w.-]+\/[\w.-]+$/.test(info.repo))throw Error('插件仓库无效。');const release=await json_request('https://api.github.com/repos/'+info.repo+'/releases/latest');version_parts(release.tag_name);const asset=release.assets?.find(item=>item.name==='plugin.zip');const expected_prefix='https://github.com/'+info.repo+'/releases/download/';if(!asset?.browser_download_url?.startsWith(expected_prefix))throw Error('插件发行包无效。');return {id:info.id,repo:info.repo,version:release.tag_name,url:asset.browser_download_url};}
 async function install_online(info){const release=await latest(info),archive=path.join(root,'download-'+crypto.randomUUID()+'.zip');try{await request(release.url,{file:archive,timeout_ms:60000});return await install_archive(archive,release);}finally{fs.rmSync(archive,{force:true});}}
