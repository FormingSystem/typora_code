...
export function create_community_plugin_settings(list:()=>any[],on_change:()=>void,navigate:(mode:string)=>void=()=>{}){
  ...
  let mount:{host:HTMLElement;on_navigate:()=>void}|undefined;
  ...
  return {
    ...
    mount(host:HTMLElement,on_navigate:()=>void){surface?.close(false);const target={host,on_navigate};mount=target;open();return()=>{if(mount!==target)return;surface?.close(false);mount=undefined;};},
    ...
    register(id:string,tab:any){
      ...
      return()=>{if(removed)return;removed=true;if(active?.tab===tab){if(mount)detach();else surface?.close(false);}try{tab.unload();}finally{tab.containerEl.remove();registered!.delete(tab);if(!registered!.size)tabs.delete(id);render();on_change();}};
    },
    dispose(){surface?.close(false);mount=undefined;tabs.clear();},
  };
}
