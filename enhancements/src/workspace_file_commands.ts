...
export function bind_workspace_file_commands(files:workspace_file_host,changed:()=>void){
  ...
  const commands:[string,string,(...args:any[])=>unknown][]=[
    ...
    ["open_folder_path","打开最近文件夹",picker.set_folder],["close_folder","关闭文件夹",picker.close_folder],
    ...
    ["close_editor","关闭编辑器",()=>{const leaf=core.app.workspace.activeLeaf;if(leaf)workspace_leaf_tab(leaf)?.querySelector<HTMLElement>(".typ-close")?.click();}],
  ];
  try{for(const[id,title,callback]of commands)lifetime.add(core.app.commands.register({id:"linux_note:"+id,title:"文件："+title,scope:"global",showInCommandPanel:id!=="open_folder_path",callback:(...args)=>run(()=>callback(...args))}));}
  catch(error){lifetime.dispose();throw error;}
  return {open_folder:()=>run(picker.open_folder),dispose:lifetime.dispose};
}
