...
export class git_source_control {
  ...
  more_menu(event: MouseEvent): void {
    ...
    panel.configured_menu(event, "source_control", [
      ...
      {id: "checkout", title: text("scm.checkout"),action:()=>panel.branch_picker.open()},
      ...
      submenu(text("scm.branches_section"), [{id: "checkout", title: text("scm.checkout"),action:()=>panel.branch_picker.open()}, {id: "branch_create", title: text("scm.create_branch"), action: () => panel.action_dialog("branch_create", "commit", "", panel.state?.head)}, ...branches]),
