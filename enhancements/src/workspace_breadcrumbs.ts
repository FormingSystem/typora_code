...
export function bind_workspace_breadcrumbs(core:graph_core,files:workspace_file_host,outline?:{current_heading?():HTMLElement|undefined;select_heading?(heading:HTMLElement):void}){
  ...
  const render=(state:group_state)=>{
    ...
    if(settings.show_editor_type){const diff=state.leaf.view?.editor?.data?.right!=null;
      if(diff){const mode=el("button");mode.append(el("span","workspace-breadcrumb-editor-label","差异编辑器"),git_icon("chevron-down"));mode.onclick=()=>{const r=mode.getBoundingClientRect(),editor=state.leaf.view.editor;workspace_menu(new MouseEvent("contextmenu",{clientX:r.left,clientY:r.bottom}),[{title:"内联显示",action:()=>editor.set_side_by_side(false)},{title:"并排显示",action:()=>{editor.inline_when_narrow=false;editor.set_side_by_side(true);}}]);};state.type.append(mode);}
      else if(/\.(md|markdown|mdown)$/i.test(state.file)){const mode=el("button");mode.append(el("span","workspace-breadcrumb-editor-label",state.model?"文本编辑器":state.leaf.view?.isEditor?.()?"Markdown 编辑器":"Markdown 预览"),git_icon("chevron-down"));mode.onclick=()=>{const leaf=state.leaf,r=mode.getBoundingClientRect();workspace_menu(new MouseEvent("contextmenu",{clientX:r.left,clientY:r.bottom}),[{title:"Markdown 编辑器",checked:!state.model,action:()=>{void files.reopen_leaf(leaf,false).catch(e=>error(String(e)));}},{title:"文本编辑器",checked:!!state.model,action:()=>{void files.reopen_leaf(leaf,true).catch(e=>error(String(e)));}}]);};state.type.append(mode);}
    }
  };
  const refresh=()=>{
