...
export function bind_terminal_workspace(host: graph_host) {
  ...
  class terminal_view extends core.WorkspaceView {
    ...
    onOpen() {
      ...
      requestAnimationFrame(() => { this.resize(); this.term?.focus(); });
    }
    async start() {
      ...
      try {
        ...
        const pty = await start_terminal_pty({ child_process: runtime.reqnode("child_process"), process_api: host.process_api, broker, executable: host.path_api.join(runtime._options.userDataPath, "linux_note_enhancements", "terminal_runtime", "node", node_release.version, "node.exe") },
      ...
      } catch (error) { this.containerEl.dataset.state = "error"; this.status.textContent = "启动失败：" + String(error); } finally { this.starting = false; }
    }
    ...
    dispose() { if (this.disposed) return; this.disposed = true; this.stop(); this.observer?.disconnect(); this.term?.dispose(); sessions.delete(this.leaf); }
    onClose() { this.active = false; setTimeout(() => { let exists = false; core.app.workspace.eachLeaves(leaf => { if (leaf === this.leaf) exists = true; }); if (!exists) this.dispose(); }, 0); }
  }
  core.app.viewManager.registerView(TERMINAL_TYPE, leaf => new terminal_view(leaf));
  ...
  core.app.workspace.ribbon.addButton({ id: "linux_note:terminal", title: "仓库终端（Ctrl+`）", group: "bottom", icon: el("i", "fa fa-terminal"), onclick: () => launch() });
