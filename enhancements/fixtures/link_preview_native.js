...
(async()=>{
 ...
 const wait=async(fn,label)=>{for(let i=0;i<100;i++){if(fn())return;await pause(50);}throw Error(label);};
 ...
 try{
  ...
  core.app.commands.run('linux_note:search');await pause(100);
  ...
  const link=select();
  const sidebar=document.querySelector('.linux-note-workspace-search');
  ...
  for(let i=0;i<20;i++){
   ...
   if(i===0){const button=[...leaf.view.containerEl.querySelectorAll('button')].find(n=>n.textContent==='打开源文件');button.click();await wait(()=>files.current_file()===target&&File.bundle.filePath===target&&!File.isFileLoading()&&!button.disabled,'源文件未进入普通编辑器');assert(!core.app.workspace.activeLeaf.state.path.startsWith('typ://linux_note.link_preview'),'源文件入口进入可编辑文档');await files.close_leaf(core.app.workspace.activeLeaf);}
