...
(async()=>{
 ...
 try {
  ...
  update_service.check_update=(_release,options)=>new Promise((resolve,reject)=>pending_updates.push({resolve,reject,signal:options.signal}));
  const checking_popup=()=>document.querySelector('[role="dialog"][aria-label="检查 Typora Code 更新"]');
  core.app.commands.run('typora_code:check_update');
  assert(checking_popup()?.querySelector('[role="status"]')?.textContent==='正在检查更新…','TC-update-native: 手动入口当帧显示正在检查');
  for(let i=0;i<20;i++)core.app.commands.run('typora_code:check_update');
  assert(pending_updates.length===1&&document.querySelectorAll('.git-graph-dialog-shade[role="dialog"]').length===1,'TC-update-native: 重复点击只执行一个请求');
  await pause(150);
  const checking_rect=checking_popup().getBoundingClientRect();
  assert(checking_rect.width>0&&checking_rect.height>0&&getComputedStyle(checking_popup()).visibility==='visible','TC-update-native: 原生样式下检查提示可见');
  checking_popup().querySelector('button').click();
  assert(pending_updates[0].signal.aborted&&!checking_popup(),'TC-update-native: 取消立即释放检查');
  core.app.commands.run('typora_code:check_update');pending_updates[0].resolve(null);await pause(60);
  assert(checking_popup()&&!pending_updates[1].signal.aborted,'TC-update-native: 迟到完成不能覆盖重试');
  pending_updates[1].reject(Error('更新服务器响应超时。'));await pause(60);
  const failed_popup=document.querySelector('[role="dialog"][aria-label="检查更新失败"]');
  assert(failed_popup?.textContent.includes('超时')&&failed_popup.textContent.includes('重试'),'TC-update-native: 超时有失败与重试提示');
  failed_popup.querySelector('button').click();
  core.app.commands.run('typora_code:check_update');pending_updates[2].resolve(null);await pause(60);
  const latest_popup=document.querySelector('[role="dialog"][aria-label="Typora Code 更新"]');
  assert(latest_popup?.textContent.includes('最新发布版本'),'TC-update-native: 检查完成给出明确结果');latest_popup.querySelector('button').click();
