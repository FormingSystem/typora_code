...
(() => {
  ...
  const run = async () => {
    ...
    try {
      ...
      await wait(() => File.editor.library.isFileTabShown()); expect(true, 'switching to files keeps sidebar open');
      app.workspace.ribbon.clickButton('core.file-explorer');
      await wait(() => !app.workspace.sidebar.isShown); expect(true, 'same file icon collapses sidebar');
      app.workspace.ribbon.clickButton('core.file-explorer');
      await wait(() => File.editor.library.isFileTabShown());
      ...
      expect(document.querySelectorAll('.git-scm-history-commit svg circle').length === 4, 'sidebar history includes graph nodes');
      document.querySelector('.git-scm-history-commit svg circle').dispatchEvent(new MouseEvent('click', {bubbles:true}));
      ...
      expect(graph_tab?.querySelector('.typ-file-icon.fa-code-fork') && !graph_tab.querySelector('.typ-file-icon.fa-file-o'), 'Git Graph tab has one correct branch icon');
