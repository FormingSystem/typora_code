...
(async () => {
  ...
  try {
    ...
    let last = performance.now();phase='initial';timer=setInterval(()=>{const now=performance.now();intervals.push(now-last);if(now-last>50)long_intervals.push({phase,duration_ms:now-last});last=now;},8);
    ...
    assert(panel.state.changes.length === 100000, '真实Git识别十万未跟踪文件');
    ...
    for (let round = 0; round < 20; round++) {
      ...
      assert(panel.state.changes.some(file=>file.path===(round%2?'large/file-00000.md':'large/changed.md')), '刷新接收实际文件变化 '+round);
    ...
    for (const zoom of [0.8, 1, 1.25]) {
      ...
      assert(!!panel.workbench.groups.querySelector('[data-file="large/file-99999.md"]'), '缩放后可访问最后文件 ' + zoom);
    }
