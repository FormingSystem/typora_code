...
const sidebar={isShown:false,panels:[],activePanel:undefined,addPanel(panel){this.panels.push(panel);return()=>this.removePanel(panel);},removePanel(panel){panel.hide?.();panel.remove_ribbon?.();this.panels=this.panels.filter(item=>item!==panel);},switch(type){const next=this.panels.find(item=>item instanceof type);if(this.activePanel===next){this.toggle();return;}this.activePanel?.hide?.();this.activePanel=next;this.show();},show(){this.isShown=true;this.activePanel?.show();},hide(){this.isShown=false;this.activePanel?.hide();},toggle(){this.isShown?this.hide():this.show();}};
for(const id of ['core.file-explorer','core.outline']){const panel=new sidebar_panel();panel.ribbonButton={id};panel.containerEl=document.createElement('div');sidebar.panels.push(panel);}
window.fixture_settings={get(key){if(key==='internalPlugin.enabledPlugins')return {'internal.workspace':true};return {openLinkInCurrentWin:true,useAutoSwap:true,hideExtensionInFileTab:false}[key];},set(){}};
window.fixture_plugin_base=class{addStatusBarItem(){const item=document.createElement("span");document.querySelector("footer").append(item);return item;}unload(){this.onunload?.();}};
window.fixture_plugin_id='forming_system.linux_note_enhancements';
window.fixture_manager={enabled:true,mutations:0};
for(const name of ['disablePlugin','unloadPlugin','uninstallPlugin','updatePlugin'])fixture_manager[name]=function(){try{plugin.onunload();}catch{}this.enabled=false;this.mutations++;};
window.fixture_manager_originals=Object.fromEntries(['disablePlugin','unloadPlugin','uninstallPlugin','updatePlugin'].map(name=>[name,fixture_manager[name]]));
window.fixture_core={Plugin:fixture_plugin_base,WorkspaceView:workspace_view,SidebarPanel:sidebar_panel,Notice:class{},app:{coreVersion:'2.10.15',plugins:fixture_manager,settings:undefined,
...
window.fixture_native_leaf=native_leaf;
